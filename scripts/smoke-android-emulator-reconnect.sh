...
def has_session_marker(node):
    return any(
        child.attrib.get("text") in {"fw_reconnect_session", "ANDROID_RECONNECT_READY"}
...
cleanup() {
  ...
  if [[ "$status" -ne 0 && "${FIELDWORK_KEEP_ANDROID_RECONNECT_TMP:-}" == "true" ]]; then
    echo "Preserving Android emulator reconnect smoke state at $tmp_dir" >&2
  else
...
cat >"$bin/fw_reconnect_session" <<EOF
#!/usr/bin/env bash
...
while IFS= read -r line; do
  ...
  if [[ "\$line" == "trigger_offline_output" ]]; then
    (
      sleep 3
      printf 'ANDROID_RECONNECT_OFFLINE_OUTPUT\n'
    ) &
  fi
done
EOF
chmod +x "$bin/fw_reconnect_session"
...
desktop_env "$fieldwork" new fw_reconnect_session >"$tmp_dir/new-session.log"
...
if ! grep -q '^Status: ok$' "$tmp_dir/launch.log"; then
  echo "Android emulator reconnect smoke launch did not report Status: ok" >&2
...
for _ in {1..60}; do
  ...
  if grep -Eq 'text="fw_reconnect_session|ANDROID_RECONNECT_READY' "$paired_ui" && ! grep -q 'text="No sessions"' "$paired_ui"; then
    break
  fi
  sleep 1
done
...
if ! grep -Eq 'text="fw_reconnect_session|ANDROID_RECONNECT_READY' "$paired_ui" || grep -q 'text="No sessions"' "$paired_ui"; then
  echo "Android emulator reconnect smoke did not show the desktop-created session." >&2
...
for _ in {1..60}; do
  ...
  if grep -q 'text="Attached"' "$terminal_ui" && grep -q 'fw_reconnect_session' "$terminal_ui"; then
    break
  fi
  sleep 1
done
...
if ! grep -q 'text="Attached"' "$terminal_ui" || ! grep -q 'fw_reconnect_session' "$terminal_ui"; then
  echo "Android emulator reconnect smoke did not attach to the desktop-created terminal session." >&2
...
adb -s "$serial" shell input text before_reconnect_ok
adb -s "$serial" shell input keyevent ENTER
wait_for_input_log before_reconnect_ok

adb -s "$serial" shell input tap 540 1200
adb -s "$serial" shell input text trigger_offline_output
adb -s "$serial" shell input keyevent ENTER
wait_for_input_log trigger_offline_output

if ! network_ping_ok; then
  echo "Android emulator reconnect smoke expected network before airplane-mode toggle." >&2
  exit 1
fi
...
sleep 5
if network_ping_ok; then
  echo "Android emulator reconnect smoke could not force network loss with airplane mode." >&2
  exit 1
fi

adb -s "$serial" shell cmd connectivity airplane-mode disable >/dev/null
for _ in {1..30}; do
  if network_ping_ok; then
    break
  fi
...
if ! network_ping_ok; then
  echo "Android emulator reconnect smoke network did not recover after airplane mode." >&2
  exit 1
fi
...
for _ in {1..60}; do
  ...
  if grep -q 'text="Attached"' "$terminal_ui"; then
    break
  fi
  sleep 1
done
if ! grep -q 'text="Attached"' "$terminal_ui"; then
  echo "Android emulator reconnect smoke terminal did not return to Attached after network restore." >&2
  sed -n '1,120p' "$terminal_ui" >&2 || true
  exit 1
fi

adb -s "$serial" shell input tap 540 1200
adb -s "$serial" shell input text after_reconnect_ok
adb -s "$serial" shell input keyevent ENTER
wait_for_input_log after_reconnect_ok
...
desktop_env "$fieldwork" pair-test \
  ...
  --expect-output "ANDROID_RECONNECT_OFFLINE_OUTPUT" \
  --expect-output "android-reconnect: after_reconnect_ok" \
...
if ! wait "$verifier_pid"; then
  verifier_pid=""
  echo "Android emulator reconnect smoke did not verify replayed network-gap output." >&2
  cat "$tmp_dir/verifier-pair.log" >&2 || true
  cat "$tmp_dir/verifier.log" >&2 || true
  adb -s "$serial" exec-out screencap -p >"$tmp_dir/android-reconnect-failure.png" || true
  adb -s "$serial" logcat -d | grep -E "FieldworkRepository|FATAL EXCEPTION|ANR in $package|app.fieldwork.android" >&2 || true
  exit 1
fi
verifier_pid=""
...
if grep -q "$package" "$crash_log"; then
  echo "Android emulator reconnect smoke found $package in the crash log." >&2
...
if grep -Eq "FATAL EXCEPTION.*$package|ANR in $package|am_crash.*$package|am_anr.*$package" "$full_log"; then
  echo "Android emulator reconnect smoke found a Fieldwork crash or ANR in logcat." >&2
  grep -E "FATAL EXCEPTION.*$package|ANR in $package|am_crash.*$package|am_anr.*$package" "$full_log" >&2
  exit 1
fi

echo "android emulator reconnect smoke ok: serial=$serial"
