...
const scaffold = path.join(root, "scripts/create-android-restart-restore-evidence-dir.mjs");
const verifier = path.join(root, "scripts/verify-android-restart-restore-evidence.mjs");
const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fieldwork-android-restart-scaffold-test-"));
...
try {
  ...
  expectEqual(manifest.schema, "fieldwork-android-restart-restore-evidence-v1", "manifest schema should be pinned");
  ...
  expect(readme.includes("FIELDWORK_ANDROID_RESTART_CAPTURE_SEED=true"), "README should document seed capture mode");
  expect(readme.includes("FIELDWORK_ANDROID_RESTART_DAEMON=true"), "README should document daemon restart mode");
  expect(readme.includes("FIELDWORK_ANDROID_RESTART_CAPTURE_APP=true"), "README should document app capture mode");
  expect(readme.includes("FIELDWORK_ANDROID_RESTART_VERIFY=true"), "README should document verify mode");
  expect(readme.includes("It does not create\n`restart-replay.txt`"), "README should state restart replay is not fabricated");
  ...
  expect(staticPreflight.stdout.includes("Android restart-restore preflight ok"), "preflight should report static success");
  expect(!fs.existsSync(path.join(evidenceDir, "restart-replay.txt")), "preflight must not fabricate restart replay");

  const seed = runPreflight(preflightPath, binDir, artifactSigning, buildConfig, {
    FIELDWORK_ANDROID_RESTART_CAPTURE_SEED: "true",
  });
  expectStatus(seed, 0, "seed capture should collect sessions and devices");
  expect(seed.stdout.includes("seed capture ok"), "seed capture should report success");
  for (const file of ["sessions-before.txt", "devices.txt"]) {
    expect(fs.existsSync(path.join(evidenceDir, file)), `seed capture should write ${file}`);
  }

  const restart = runPreflight(preflightPath, binDir, artifactSigning, buildConfig, {
    FIELDWORK_ANDROID_RESTART_DAEMON: "true",
  });
  expectStatus(restart, 0, "daemon restart capture should run fw daemon restart and record timing");
  expect(restart.stdout.includes("daemon restart capture ok"), "daemon restart capture should report success");
  expect(
    fs.readFileSync(path.join(evidenceDir, "daemon-restart.txt"), "utf8").includes("processes_died_documented"),
    "daemon-restart.txt should document process-death semantics",
  );

  const app = runPreflight(preflightPath, binDir, artifactSigning, buildConfig, {
    FIELDWORK_ANDROID_RESTART_CAPTURE_APP: "true",
  });
  expectStatus(app, 0, "app restore capture should collect Android restart evidence");
  expect(app.stdout.includes("app capture ok"), "app restore capture should report success");
  for (const file of ["restart.png", "restart-ui.xml", "restart-logcat.log", "restart-crash.log"]) {
  ...
  const verifyMissingReplay = runPreflight(preflightPath, binDir, artifactSigning, buildConfig, {
    FIELDWORK_ANDROID_RESTART_VERIFY: "true",
  });
  expectStatus(verifyMissingReplay, 1, "verify mode should require real restored replay evidence");
  expect(verifyMissingReplay.stderr.includes("restart-replay.txt is missing"), "missing replay failure should be explicit");

  fs.writeFileSync(path.join(evidenceDir, "restart-replay.txt"), "fw_restart_session\nANDROID_RESTART_SCROLLBACK\n");
  const verifyComplete = runPreflight(preflightPath, binDir, artifactSigning, buildConfig, {
    FIELDWORK_ANDROID_RESTART_VERIFY: "true",
  });
  expectStatus(verifyComplete, 0, "verify mode should pass after staged evidence and replay transcript exist");
  expect(verifyComplete.stdout.includes("Android restart-restore evidence ok"), "verify mode should report restart-restore evidence success");
  ...
  console.log("Android restart-restore evidence scaffold self-test ok");
} finally {
...
if (args[0] === "ls") {
  process.stdout.write("019e0000-0000-7000-8000-000000000001\\tfw_restart_session\\tIdle\\tbash\\n");
  process.exit(0);
}
if (args[0] === "devices") {
...
if (args[0] === "daemon" && args[1] === "restart") {
  process.stdout.write("fieldworkd restarted\\n");
  process.exit(0);
}
...
if (args[0] === "logcat") {
  ...
  else out("I FieldworkRepository: listSessions returned 1 sessions\\n");
  process.exit(0);
}
if (args[0] === "shell" && args[1] === "am") process.exit(0);
...
if (args[0] === "pull") {
  fs.writeFileSync(args[2], '<hierarchy><node text="fw_restart_session"/><node text="Attached"/></hierarchy>\\n');
