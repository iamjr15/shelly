...
function verifyPromptToArtifactChecklist() {
  ...
  requireText(
    audit,
    "Android session-subscription evidence verifier fixture test",
    "release audit must record local release aggregate coverage for Android session-subscription evidence verifier self-test",
  );
  requireText(
    audit,
  ...
  requireText(
    audit,
    "Android resize/detach evidence verifier fixture test",
    "release audit must record local release aggregate coverage for Android resize/detach evidence verifier self-test",
  );
  requireText(
    audit,
  ...
  requireText(
    audit,
    "Android restart-restore evidence verifier fixture test",
    "release audit must record local release aggregate coverage for Android restart-restore evidence verifier self-test",
  );
  requireText(
    audit,
  ...
  requireText(
    audit,
    "docs/ANDROID_RENDERER.md`, `docs/ANDROID_PAIR_FLOW.md`, `docs/ANDROID_SESSION_SUBSCRIPTION.md`, `docs/ANDROID_TERMINAL_ATTACH.md`, `docs/ANDROID_RESIZE_DETACH.md`, `docs/ANDROID_BIOMETRIC.md`, `docs/ANDROID_DOGFOOD.md`, `docs/ANDROID_COLD_START.md`, `docs/ANDROID_RENDERER_FLOOD.md`, `docs/ANDROID_BACKGROUND_FOREGROUND.md`, `docs/ANDROID_NETWORK_RECONNECT.md`, `docs/ANDROID_RESTART_RESTORE.md`, `docs/ANDROID_MULTISESSION.md`, `docs/ANDROID_FCM_PUSH.md`, `docs/MACOS_DAEMON_SURVIVAL.md`, `docs/LIVE_TESTING.md`, `docs/OPERATIONS.md`, and `docs/RELEASE_AUDIT.md`",
  ...
  requireText(
    audit,
    "operator npm/secret handoff",
    "release audit docs-sync row must record operations handoff coverage",
  );
  requireText(
    audit,
  ...
  requireText(
    audit,
    "latest pass from `pnpm check:local-release:full` measured CLI max `3.85ms` and daemon max `44.55ms`",
  ...
  requireText(
    audit,
    "current v1 install, protocol, privacy, architecture, Android renderer, Android pair flow, Android session subscription, Android terminal attach, Android resize/detach, Android biometric, Android dogfood, Android cold-start, Android renderer flood, Android background/foreground, Android network reconnect, Android restart restore, Android multisession, Android FCM push, macOS daemon survival, first live-test, operator npm/secret handoff, iOS blocker, mobile-boundary, npm-only distribution, and deferred-scope facts",
  ...
  requireText(
    ...
    "release audit must record the Android pair-flow runbook",
  );
  requireText(
    audit,
    "`scripts/verify-android-pair-flow-evidence.mjs` fixture coverage",
    "release audit must record Android pair-flow evidence verifier fixture coverage",
  );
  requireText(
    audit,
  ...
  requireText(
    ...
    "release audit must record the Android session-subscription runbook",
  );
  requireText(
    audit,
  ...
  requireText(
    ...
    "release audit must record the Android terminal attach runbook",
  );
  requireText(
    audit,
    "`scripts/verify-android-terminal-attach-evidence.mjs` fixture coverage",
    "release audit must record Android terminal attach evidence verifier fixture coverage",
  );
  requireText(
    audit,
  ...
  requireText(
    ...
    "release audit must record the Android resize/detach runbook",
  );
  requireText(
    audit,
  ...
  requireText(
    audit,
    "release-device resize and detach/reattach replay evidence",
    "release audit must record the Android resize/detach evidence contract",
  );
  requireText(
    audit,
  ...
  requireText(
    ...
    "release audit must record the Android restart-restore runbook",
  );
  requireText(
    audit,
  ...
  requireText(
    ...
    "release audit must record the Android background/foreground runbook",
  );
  requireText(
    audit,
    "`scripts/verify-android-background-foreground-evidence.mjs` fixture coverage",
    "release audit must record Android background/foreground evidence verifier fixture coverage",
  );
  requireText(
    audit,
...
function verifyLatestRefresh() {
  ...
  requireText(audit, "`pnpm check:local-release:full`", "release audit must record the latest artifact/runtime aggregate command");
  ...
  requireText(audit, "`3.33ms`, p95 `3.81ms`, max `3.85ms`", "release audit must record latest CLI desktop performance values");
  requireText(audit, "`42.43ms`, p95 `44.52ms`, max `44.55ms`", "release audit must record latest daemon desktop performance values");
  ...
  requireText(audit, "README pointer to the operator-facing\nrelease-gate handoff", "release audit must record README operator-handoff pointer coverage");
  requireText(audit, "`PLAN.md` completion-checkbox source of truth", "release audit must record README PLAN.md completion-checkbox source coverage");
  ...
  requireText(audit, "nine hooks as always-run workspace/security/lightweight release gates", "release audit must record pre-commit workspace/security/lightweight release coverage");
  requireText(audit, "node scripts/test-live-testing-evidence.mjs", "release audit must record pre-commit live-testing evidence hook coverage");
  requireText(audit, "node scripts/test-debug-instance.mjs", "release audit must record pre-commit debug-instance hook coverage");
  requireText(audit, "node scripts/verify-structured-assets.mjs", "release audit must record pre-commit structured-assets hook coverage");
  ...
  for (const evidence of [
    ...
    "`TotalTime=4572ms`",
    "`locked.png`, `locked-ui.xml`,\n`locked-logcat.log`, empty `locked-crash.log`, and `buildconfig.txt`",
    "`APPLICATION_ID = \"app.fieldwork.android\"`",
    "`BUILD_TYPE = \"debug\"`",
    "`DEBUG = Boolean.parseBoolean(\"true\")`",
    "app process remained focused",
    "no Fieldwork `FATAL EXCEPTION` or ANR\nentries",
    "physical release\ndevice cold-start and biometric evidence remain unchecked",
  ]) {
    requireText(plan, evidence, `PLAN.md 2026-05-22 direct adb locked-launch evidence must include ${evidence}`);
  }
  for (const evidence of [
    "follow-up 2026-05-22 direct adb locked-launch refresh",
    "`/tmp/fieldwork-adb-refresh-20260522`",
    "`app.fieldwork.android/.MainActivity`",
    "`TotalTime=4572ms`",
    "`APPLICATION_ID = \"app.fieldwork.android\"`",
...
function verifyVerifierIsWired() {
  ...
  requireText(localHandoff, "run_fieldwork_new \"$tmp/new-reconnect.log\" --name FW_RECONNECT_READY", "local handoff smoke must name the reconnect session to avoid duplicate-name collisions");
