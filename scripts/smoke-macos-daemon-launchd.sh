...
cleanup() {
  local status=$?
  set +e
  if [[ "$service_installed" == "1" && -n "${fieldwork:-}" && -x "${fieldwork:-}" ]]; then
    "$fieldwork" daemon uninstall >"$tmp/daemon-uninstall.log" 2>&1
  fi
  if [[ -n "${fieldworkd:-}" ]]; then
    pkill -KILL -x fieldworkd >/dev/null 2>&1
  fi
  if launchctl print "gui/$(id -u)/app.fieldwork.daemon" >/dev/null 2>&1; then
    launchctl bootout "gui/$(id -u)/app.fieldwork.daemon" >/dev/null 2>&1
  fi
  if [[ "$launchd_env_set" == "1" ]]; then
    launchctl unsetenv FIELDWORK_IROH_SECRET_KEY_B64 >/dev/null 2>&1
  fi
  if [[ "${FIELDWORK_SMOKE_KEEP_TMP:-}" == "1" || "$status" != "0" ]]; then
...
if launchctl print "gui/$(id -u)/app.fieldwork.daemon" >/dev/null 2>&1; then
  echo "app.fieldwork.daemon is already loaded; refusing to disturb an existing user service" >&2
  exit 1
fi

if pgrep -x fieldworkd >/dev/null 2>&1; then
  echo "fieldworkd is already running; stop it before running the launchd smoke" >&2
  pgrep -fl fieldworkd >&2 || true
  exit 1
fi
...
export FIELDWORK_LOG_DIR="$tmp/logs"
...
if [[ ! -x "$platform_dir/bin/fieldwork" || ! -x "$platform_dir/bin/fieldworkd" ]]; then
...
fw="$tmp/project/node_modules/.bin/fw"

for binary in "$fieldwork" "$fieldworkd" "$fw"; do
  if [[ ! -x "$binary" ]]; then
...
launchctl setenv FIELDWORK_IROH_SECRET_KEY_B64 "BQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQUFBQU"
launchd_env_set=1

{
  echo "$fieldwork daemon install"
...
if grep -Fq "FIELDWORK_IROH_SECRET_KEY_B64" "$plist"; then
