...
function verifyAndroidPairFlowDoc() {
  for (const needle of [
    ...
    "Use exactly one physical Android phone",
...
function verifyAndroidBiometricDoc() {
  for (const needle of [
    ...
    "Use exactly one physical Android phone with biometrics enrolled",
    "not an emulator or\n  AVD",
    "debug build, biometric bypass, or debug pairing payload",
    "real QR scanner and explicit desktop approval",
    "USB debugging; end users do not need adb",
...
function verifyReadme() {
  for (const needle of [
    ...
    "appears as the active session name in the mobile app dashboard",
    "The daemon rejects duplicate session names",
...
function verifyLiveTestingDoc() {
  for (const needle of [
    ...
    "captured UI dumps do not show Android system\nnot-responding overlays",
    "captured logcat files do not contain any Android fatal\nor ANR entries",
    "every captured crash-buffer file is empty after\n`adb logcat -c`",
    "echo android_live_ok",
...
function verifyInstallDoc() {
  for (const needle of [
    ...
    "create and attach a new default `claude` session with a generated\none-word name like `waffle` or `kazoo`",
    "even when other sessions already exist",
    ...
    "Running either CLI name with no subcommand uses the no-args fast path",
    "auto-generated one-word\ndisplay name that mobile apps show from the daemon session list",
    "Running `fw refactoringjob` uses the named-session fast\npath",
...
function verifyPlanDoc() {
  for (const needle of [
    ...
    "**No-args fast path**",
    "generated one-word display name such as `waffle`\nor `kazoo`",
    "`SessionSummary.name`, so mobile apps show the same\nactive session name in the dashboard",
