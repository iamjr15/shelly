...
function verifyAndroidRendererDoc() {
  for (const needle of [
    ...
    "Latest hosted-relay aggregate run on 2026-05-29 passed on `emulator-5554`",
    "`TotalTime=6448ms`",
    "`pair_flow_ms=1420`",
    "`visible_ms=5493`",
    "8437/14400 flood screenshot nonblack samples",
...
function verifyAndroidPairFlowDoc() {
  for (const needle of [
    ...
    "`pair_flow_ms<=15000`",
    "either scanning the QR ticket or typing the\n5-character code — plus explicit desktop approval in `pair_flow_ms<=15000`",
    "QA-only use of USB debugging; end users do not need adb",
    "Use exactly one physical Android phone",
    "not an emulator or AVD",
    "debug build, biometric bypass, or the debug pairing code",
    "real QR scanner (or relay-backed manual code entry) and\n  explicit desktop approval",
    "direct `adb`",
    "pnpm scaffold:android-pair-flow-evidence -- --dir \"$FW_ANDROID_PAIR_DIR\"",
    "direct-adb `preflight.sh`",
    ...
    "only proves the Android release-device pair (QR scan or\nrelay-backed code entry) and dashboard path",
  ]) {
    requireText(
...
function verifyAndroidSessionSubscriptionDoc() {
  for (const needle of [
    ...
    "Use exactly one physical Android phone",
    "not an emulator or AVD",
    "debug build, biometric bypass, or debug pairing code",
...
function verifyAndroidTerminalAttachDoc() {
  for (const needle of [
    ...
    "direct-adb `preflight.sh`",
    "does not create desktop sessions or PTY replay transcripts",
...
function verifyAndroidResizeDetachDoc() {
  for (const needle of [
    ...
    "direct-adb `preflight.sh`",
    "does not create desktop sessions, change Android commands, or\ncreate PTY replay transcripts",
...
function verifyAndroidDogfoodDoc() {
  for (const needle of [
    ...
    "direct-adb `preflight.sh`",
    "does not create dogfood duration proof, human-review\nmarkers, or PTY replay transcripts",
...
function verifyAndroidRendererFloodDoc() {
  for (const needle of [
    ...
    "direct-adb `preflight.sh`",
    "does not create desktop sessions, run the flood\ncommand, or create `flood-replay.txt`",
...
function verifyAndroidMultisessionDoc() {
  for (const needle of [
    ...
    "three per-session replay files",
    "node scripts/verify-android-aab.mjs --expect-signed",
    "signed release bundle ok",
    "BUILD_TYPE = \"release\"",
    "DEBUG = false",
...
function verifyAndroidFcmPushDoc() {
  for (const needle of [
    ...
    "direct-adb `preflight.sh`",
    "does not create provider payload JSON",
    "FIELDWORK_ANDROID_AAB",
    "FIELDWORK_ANDROID_ARTIFACT_SIGNING_FILE",
    "node scripts/verify-android-aab.mjs --expect-signed",
    "FIELDWORK_BIOMETRIC_BYPASS = false",
    'FIELDWORK_DEBUG_PAIRING_CODE = ""',
    "adb shell pm path app.fieldwork.android",
...
function verifyRelayHoneycombDoc() {
  for (const needle of [
    ...
    "non-secret `preflight.sh`",
    "FIELDWORK_RELAY_VERSION_URL",
    "FIELDWORK_RELAY_RECEIPT_TEST_WINDOW=true",
    "does not export Honeycomb query rows",
    "relay-version.txt",
    "relay-config.txt",
...
function verifyMacosDaemonSurvivalDoc() {
  for (const needle of [
    ...
    'fieldwork new --dir "$FW_MACOS_PROJECT_DIR" --name macos_sleep -- bash -lc',
    "MACOS_SLEEP_SCROLLBACK_BEFORE",
    "sleep_duration_ms",
    "after_sleep_wake_ok",
    'fieldwork new --dir "$FW_MACOS_PROJECT_DIR" --name macos_kill -- bash -lc',
    "MACOS_KILL_SCROLLBACK_BEFORE",
    "restart_ms",
    "processes_died_documented",
    "[fieldwork: session exited",
    "daemon-log.txt",
...
function verifyOperationsDoc() {
  for (const needle of [
    ...
    "ad-hoc-signed Darwin artifacts",
    "`codesign --force --sign -`",
    "no quarantine xattr",
    "pnpm scaffold:release-artifacts-evidence -- --print-dir",
    "pnpm check:release-artifacts-evidence -- \"$FW_RELEASE_ARTIFACT_EVIDENCE_DIR\"",
    "cosign-backed `pnpm check:release-artifacts` output",
    "pnpm scaffold:macos-signing-evidence -- --print-dir",
    ...
    "pnpm scaffold:npm-release-evidence -- --print-dir",
...
function verifyReadme() {
  for (const needle of [
    ...
    "The pairing code is a single active 5-character Crockford base32 code, case-insensitive, expires after 10 minutes, and is invalidated after 5 wrong in-band attempts",
...
function verifyLiveTestingDoc() {
  for (const needle of [
    ...
    "run the strict form after the Desktop Setup shim has put `fw` on `PATH`",
...
function verifyProtocolDoc() {
  for (const needle of [
    ...
    "single active 5-character Crockford pairing code (10-minute TTL, `OsRng`)",
    "`PairWithCode { code, device_name, device_node_id }` replaces v1's `PairWithToken`",
    "invalidates the active code after 5 wrong attempts",
...
function verifyPrivacyDoc() {
  for (const needle of [
    ...
    "The code is the only credential (desktop approval is still required), uses a confusable-free Crockford alphabet, expires after 10 minutes, and is invalidated after 5 wrong in-band attempts",
    "The QR pairing path stays entirely daemon-local: nothing about it is sent to the relay",
    "No daemon or mobile crash-reporting SDK is bundled in v1",
    "the daemon does not export crash reports or OTLP/Honeycomb traces",
    "fails if removed crash SDK classes or DSN markers are packaged",
    "v1 does not start a mobile crash-reporting SDK",
...
function verifyInstallDoc() {
  for (const needle of [
    ...
    "pnpm scaffold:npm-release-evidence -- --print-dir",
    "pnpm check:npm-release-evidence -- \"$FW_NPM_RELEASE_DIR\"",
    "post-publish evidence capture only",
...
function verifyPlanDoc() {
  for (const needle of [
    ...
    "`CONTRACT_VERSION = 2` constant in `protocol` crate",
    "**Pairing code**: exactly 5 characters from the Crockford base32 alphabet",
    "A single code is active at a time; the daemon invalidates the active code after 5 wrong in-band attempts",
    "**Pairing ticket**: the compact `PairingTicket { code, node_id, relay_url, addrs, expires_at }`",
    "a device pairs by EITHER scanning the QR OR typing the 5-character code",
    "the typed code resolves to the same reachability through the relay rendezvous endpoint",
    "**desktop approval stays the final gate.**",
    "**Scrollback/device registry encrypted at rest**",
    "encrypted devices.redb under a hashed device row key",
    "The QR pairing path remains daemon-local and is not stored by the relay",
    ...
    "hosted evidence contract and scaffold for relay config, systemd credential proof, `/v1/version` test traffic, Honeycomb query export, and redacted relay logs",
    ...
    "npm release evidence contract (2026-05-25)",
    "scripts/create-npm-release-evidence-dir.mjs",
    "scripts/verify-npm-release-evidence.mjs",
    "without publishing packages or querying package-name availability",
    "pnpm check:npm-release-evidence",
    "Release-artifact evidence contract (2026-05-25)",
    "scripts/create-release-artifacts-evidence-dir.mjs",
    "scripts/verify-release-artifacts-evidence.mjs",
    "without creating release artifacts or running GitHub workflows",
    "pnpm check:release-artifacts-evidence",
    "macOS signing evidence contract (revised 2026-05-26, tightened 2026-05-30)",
    "scripts/create-macos-signing-evidence-dir.mjs",
    "scripts/verify-macos-signing-evidence.mjs",
    "without signing binaries or running GitHub workflows",
    "ad-hoc signatures",
    "xattr -p com.apple.quarantine",
    "pnpm check:macos-signing-evidence",
