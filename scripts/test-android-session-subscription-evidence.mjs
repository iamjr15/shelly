...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), "fieldwork-android-session-subscription-"));

try {
  const good = path.join(temp, "good");
  writeFixture(good);
  expectStatus(good, 0, "good Android session-subscription evidence should pass");
  ...
  const alreadyVisible = path.join(temp, "already-visible");
  writeFixture(alreadyVisible);
  fs.writeFileSync(path.join(alreadyVisible, "dashboard-before-ui.xml"), '<hierarchy><node text="fw_live_sub"/></hierarchy>\n');
  expectStatus(alreadyVisible, 1, "pre-existing session should fail", "dashboard-before-ui.xml must prove fw_live_sub was not already visible");

  const missingDesktopCreate = path.join(temp, "missing-desktop-create");
  writeFixture(missingDesktopCreate);
  fs.writeFileSync(path.join(missingDesktopCreate, "desktop-create.txt"), "opened Android app\n");
  expectStatus(
    missingDesktopCreate,
    1,
    "missing desktop create command should fail",
    "desktop-create.txt must show the desktop CLI command fw new --name fw_live_sub bash",
  );

  const slowVisible = path.join(temp, "slow-visible");
  writeFixture(slowVisible);
  fs.writeFileSync(path.join(slowVisible, "subscription-visible.txt"), "created_by_desktop_cli\nvisible_ms=2001\n");
  expectStatus(slowVisible, 1, "slow subscription visibility should fail", "visible_ms=2001");

  const emptyDashboard = path.join(temp, "empty-dashboard");
  writeFixture(emptyDashboard);
  fs.writeFileSync(path.join(emptyDashboard, "subscription-ui.xml"), '<hierarchy><node text="No sessions"/></hierarchy>\n');
  expectStatus(emptyDashboard, 1, "empty dashboard after desktop create should fail", "subscription-ui.xml must not be the empty dashboard");

  const mobileCreateControl = path.join(temp, "mobile-create-control");
  writeFixture(mobileCreateControl);
  fs.writeFileSync(path.join(mobileCreateControl, "subscription-ui.xml"), '<hierarchy><node text="fw_live_sub"/><node text="Create session"/></hierarchy>\n');
  expectStatus(mobileCreateControl, 1, "mobile create control should fail", "must not expose mobile session creation");

  const missingLog = path.join(temp, "missing-log");
  writeFixture(missingLog);
  fs.writeFileSync(path.join(missingLog, "subscription-logcat.log"), "I FieldworkRepository: pair completed\n");
  expectStatus(missingLog, 1, "missing session-list log should fail", "session-list activity after desktop creation");

  const missingReplay = path.join(temp, "missing-replay");
  writeFixture(missingReplay);
  fs.writeFileSync(path.join(missingReplay, "subscription-replay.txt"), "fw_live_sub bash\n");
  expectStatus(
    missingReplay,
    1,
    "missing Android-originated input should fail",
  ...
  writeFixture(badLog);
  fs.writeFileSync(path.join(badLog, "subscription-logcat.log"), "FATAL EXCEPTION: main\napp.fieldwork.android crashed\n");
  expectStatus(badLog, 1, "fatal log should fail", "subscription-logcat.log must not contain Fieldwork fatal, ANR, or exception entries");
} finally {
  fs.rmSync(temp, { recursive: true, force: true });
}

console.log("Android session-subscription evidence verifier ok");
...
function writeFixture(dir) {
  ...
  fs.writeFileSync(path.join(dir, "dashboard-before-ui.xml"), '<hierarchy><node text="refactoringjob"/><node text="shell"/></hierarchy>\n');
  fs.writeFileSync(path.join(dir, "desktop-create.txt"), "$ fw new --name fw_live_sub bash\ncreated fw_live_sub bash\n");
  writePng(path.join(dir, "subscription.png"), { width: 1080, height: 2400 });
  fs.writeFileSync(
    path.join(dir, "subscription-ui.xml"),
    '<hierarchy><node text="refactoringjob"/><node text="fw_live_sub"/><node text="bash"/></hierarchy>\n',
  );
  fs.writeFileSync(
    path.join(dir, "subscription-logcat.log"),
  ...
  fs.writeFileSync(path.join(dir, "subscription-replay.txt"), "fw attach fw_live_sub\nsubscription_attach_ok\n");
  fs.writeFileSync(path.join(dir, "sessions-after.txt"), "fw_live_sub bash\nrefactoringjob claude\nshell bash\n");
}
