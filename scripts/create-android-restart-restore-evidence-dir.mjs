...
const evidenceDir = path.resolve(options.dir ?? path.join("/tmp", `fieldwork-android-restart-${timestampForDir(new Date())}`));
...
const manifest = {
  schema: "fieldwork-android-restart-restore-evidence-v1",
  createdAt: new Date().toISOString(),
  evidenceDir,
  verifier: path.relative(root, verifier),
  requiredFiles,
  generatedFiles,
  note: "This scaffold captures signed release/device proof, desktop seed/restart proof, and direct-adb Android restart-restore screenshot/UI/log evidence. It does not create the restored PTY replay transcript.",
};
...
if (options.printDir) {
  ...
  console.log(`Android restart-restore evidence scaffold created: ${evidenceDir}`);
...
function printUsage() {
  console.error("usage: node scripts/create-android-restart-restore-evidence-dir.mjs [--dir <path>] [--force] [--print-dir] [--quiet]");
}
...
function buildReadme(dir, files) {
  return `# Fieldwork Android Restart Restore Evidence

This directory is a scaffold for the Android physical release-device daemon
restart restore gate.

Evidence directory:

\`\`\`sh
export FW_ANDROID_RESTART_DIR="${dir}"
\`\`\`

Use \`docs/ANDROID_RESTART_RESTORE.md\` as the authoritative runbook. This
...
After the physical phone is paired and \`fw_restart_session\` exists, capture
the desktop seed state:

\`\`\`sh
FIELDWORK_ANDROID_RESTART_CAPTURE_SEED=true "$FW_ANDROID_RESTART_DIR/preflight.sh"
\`\`\`

After \`ANDROID_RESTART_SCROLLBACK\` has been emitted and the session has exited
so scrollback is persisted, restart the daemon and record timing:
...
The helper captures \`sessions-before.txt\`, \`devices.txt\`,
\`daemon-restart.txt\`, \`restart.png\`, \`restart-ui.xml\`,
\`restart-logcat.log\`, and \`restart-crash.log\`. It does not create
\`restart-replay.txt\`; that must come from a real desktop
\`fw attach fw_restart_session\` transcript proving restored
\`ANDROID_RESTART_SCROLLBACK\`.

After \`restart-replay.txt\` exists, run:
...
function buildCaptureChecklist(dir, files) {
  return `# Fieldwork Android Restart Restore Capture Checklist
...
The preflight helper uses direct \`adb\` and local \`fw\`, but it does not create
desktop sessions, emit scrollback, open the Android UI, or create the restored
PTY replay transcript.
...
\`\`\`sh
"$FW_ANDROID_RESTART_DIR/preflight.sh"
# Pair Android, create fw_restart_session, emit ANDROID_RESTART_SCROLLBACK, let it exit.
FIELDWORK_ANDROID_RESTART_CAPTURE_SEED=true "$FW_ANDROID_RESTART_DIR/preflight.sh"
FIELDWORK_ANDROID_RESTART_DAEMON=true "$FW_ANDROID_RESTART_DIR/preflight.sh"
FIELDWORK_ANDROID_RESTART_CAPTURE_APP=true "$FW_ANDROID_RESTART_DIR/preflight.sh"
...
evidence_dir="\${FW_ANDROID_RESTART_DIR:-$(cd "$(dirname "\${BASH_SOURCE[0]}")" && pwd)}"
...
capture_seed="\${FIELDWORK_ANDROID_RESTART_CAPTURE_SEED:-false}"
restart_daemon="\${FIELDWORK_ANDROID_RESTART_DAEMON:-false}"
capture_app="\${FIELDWORK_ANDROID_RESTART_CAPTURE_APP:-false}"
verify_evidence="\${FIELDWORK_ANDROID_RESTART_VERIFY:-false}"
...
if [[ "$capture_seed" == "true" ]]; then
  fw_cmd ls > "$evidence_dir/sessions-before.txt"
  fw_cmd devices > "$evidence_dir/devices.txt"
  chmod 0600 "$evidence_dir/sessions-before.txt" "$evidence_dir/devices.txt"
  require_fixed "$evidence_dir/sessions-before.txt" "fw_restart_session" "sessions-before.txt must include fw_restart_session"
  require_regex "$evidence_dir/devices.txt" '(Android|Pixel|phone|paired|device)' "devices.txt must show the paired Android device"
  echo "Android restart-restore seed capture ok: $evidence_dir"
elif [[ "$restart_daemon" == "true" ]]; then
  restart_start_ms="$(node -e 'console.log(Date.now())')"
  {
    echo '$ fw daemon restart'
    fw_cmd daemon restart
    restart_end_ms="$(node -e 'console.log(Date.now())')"
    printf 'restart_ms=%s\\n' "$((restart_end_ms - restart_start_ms))"
    printf 'processes_died_documented\\n'
  } > "$evidence_dir/daemon-restart.txt"
  chmod 0600 "$evidence_dir/daemon-restart.txt"
  require_fixed "$evidence_dir/daemon-restart.txt" "fw daemon restart" "daemon-restart.txt must show fw daemon restart"
  require_fixed "$evidence_dir/daemon-restart.txt" "processes_died_documented" "daemon-restart.txt must document process death"
  echo "Android restart-restore daemon restart capture ok: $evidence_dir"
elif [[ "$capture_app" == "true" ]]; then
  adb_cmd shell am force-stop app.fieldwork.android
  adb_cmd shell am start -W -n app.fieldwork.android/.MainActivity > "$evidence_dir/restart-am-start.txt"
  adb_cmd exec-out screencap -p > "$evidence_dir/restart.png"
  adb_cmd shell uiautomator dump /sdcard/fieldwork-restart.xml >/dev/null
  adb_cmd pull /sdcard/fieldwork-restart.xml "$evidence_dir/restart-ui.xml" >/dev/null
  adb_cmd shell rm /sdcard/fieldwork-restart.xml >/dev/null 2>&1 || true
  adb_cmd logcat -d > "$evidence_dir/restart-logcat.log"
  adb_cmd logcat -d -b crash > "$evidence_dir/restart-crash.log"
  chmod 0600 "$evidence_dir/restart.png" "$evidence_dir/restart-ui.xml" "$evidence_dir/restart-logcat.log" "$evidence_dir/restart-crash.log" "$evidence_dir/restart-am-start.txt"
  echo "Android restart-restore app capture ok: $evidence_dir"
elif [[ "$verify_evidence" == "true" ]]; then
  if [[ ! -f "$evidence_dir/restart-replay.txt" ]]; then
    echo "restart-replay.txt is missing; capture the real desktop fw attach transcript first" >&2
    exit 1
  fi
  node "$repo_root/scripts/verify-android-restart-restore-evidence.mjs" "$evidence_dir" >/dev/null
  echo "Android restart-restore evidence ok: $evidence_dir"
else
  adb_cmd logcat -c
  adb_cmd logcat -b crash -c
  echo "Android restart-restore preflight ok: $evidence_dir"
  echo "next: pair the phone, create fw_restart_session, capture seed, restart daemon, capture app restore, then add restart-replay.txt"
fi
`;
}
