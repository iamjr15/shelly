  ...
  try {
    ...
    const result = spawnSync(process.execPath, [path.join(installTmp, "install.js")], {
      ...
      env: {
        ...
        FIELDWORK_NPM_PLATFORM: platform,
        FIELDWORK_NPM_ARCH: arch,
        FIELDWORK_TEST_MACOS_TRUST_LOG: trustLog,
    ...
    if (platform === "darwin") {
      const installedFieldwork = fs.realpathSync(path.join(installTmp, "bin/fieldwork"));
      const installedDaemon = fs.realpathSync(path.join(installTmp, "bin/fieldworkd"));
      assert(trustCalls.length === 4, `${key} install.js should run macOS trust prep for both binaries`);
      assertToolCall(trustCalls[0], "codesign", ["--force", "--sign", "-", installedFieldwork], `${key} fieldwork codesign`);
      assertToolCall(trustCalls[1], "xattr", ["-d", "com.apple.quarantine", installedFieldwork], `${key} fieldwork quarantine cleanup`);
      assertToolCall(trustCalls[2], "codesign", ["--force", "--sign", "-", installedDaemon], `${key} fieldworkd codesign`);
      assertToolCall(trustCalls[3], "xattr", ["-d", "com.apple.quarantine", installedDaemon], `${key} fieldworkd quarantine cleanup`);
    } else {
...
if (process.env.FIELDWORK_TEST_MACOS_TRUST_LOG) {
  fs.appendFileSync(
    process.env.FIELDWORK_TEST_MACOS_TRUST_LOG,
