...
function verifyDevelopmentDoc(text) {
  ...
  for (const section of [
    ...
    "`fw` with no subcommand uses the same smart default as `fieldwork`",
    "`fw pair` starts the same QR-pairing flow as `fieldwork pair`",
    "`fw <name>` is the named-session fast path after npm install",
    "the npm dispatcher test covers smart-default, pair, named-session, and `fw completion bash` alias shapes against the platform-package fallback path",
    "forwards the invoked alias through `FIELDWORK_CLI_BIN_NAME` and `argv0`",
    "native completion generation still follows `fw` versus `fieldwork`",
  ...
  for (const phrase of [
    ...
    "app-release.aab` (`54M`, SHA-256 `875fbfb608f0eb937bbaf466f5299b111611f9d48ada8ffdc59c4bbb30c7c099`)",
