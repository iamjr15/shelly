...
@MainActor
final class ShellyCoreService {
    private let keychain: KeychainStore
    private var client: ShellyClient?
    ...
    private func requireClient() throws -> ShellyClient {
        if let client {
            return client
        }
        let restored = restoreSavedPairing()
        guard restored, let client else {
            throw ShellyAppError.notPaired
        }
        return client
    }

    private func makeClient(record: PairedDaemonRecord?) throws -> ShellyClient {
        ...
        return try ShellyClient(config: config)
    }

    /// Relay control URL injected at build time via the `ShellyRelayControlURL`
    /// Info.plist key (sourced from `SHELLY_RELAY_CONTROL_URL`). Absent or empty
    /// disables the typed-code path; QR pairing keeps working without a relay.
    private static var relayControlUrl: String? {
        guard
            let value = Bundle.main.object(forInfoDictionaryKey: "ShellyRelayControlURL") as? String
        else {
            return nil
        }
...
enum ShellyAppError: LocalizedError {
    case notPaired
