...
| Local non-external release gate | `scripts/check-local-release.mjs` and `package.json` `check:local-release` aggregate the deterministic source-side verifiers and fixture tests that do not require credentials, live publishing, iOS SDK builds, Android emulator runtime, physical devices, or hosted relay deployment. The optional `--with-artifacts` mode adds preserved AAB, staged npm binary, publish-readiness, and npm dry-run pack checks when local artifacts are present. | Locally verified |
...
1.3.13. Relay TLS and OTLP loopback smokes passed; the performance run reported CLI median `3.30ms`, p95 `3.99ms`, max
`5.57ms`, and daemon ready-to-handshake median `42.23ms`, p95 `50.77ms`, max
`54.37ms` over 25 measured release-build samples; npm binary readiness passed
...
- Desktop performance passed after one explicit warm-up sample, with CLI median
  `3.30ms`, p95 `3.99ms`, max `5.57ms`, and daemon ready-to-handshake median
  `42.23ms`, p95 `50.77ms`, max `54.37ms` over 25 measured release-build
  samples.
