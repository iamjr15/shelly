...
| First Android live-test runbook | `docs/LIVE_TESTING.md` defines the first operator-assisted Android physical-device terminal handoff pass: Android-only, not v1 release sign-off, same daemon-owned PTY session, not screen mirroring, no takeover of arbitrary already-open Terminal.app or iTerm tabs, no iOS/npm publish/store/production relay/APNs/FCM/domain/signing scope, USB debugging only for QA evidence and not an end-user requirement, direct `adb` screenshot/UI/log/crash capture, desktop-created `bash`, `claude`, and `vim`/`htop` sessions, the bare `fieldwork` auto-named default `claude` shortcut, and the `fieldwork refactoringjob` named shortcut appearing in the Android dashboard | Runbook synchronized; physical-device execution remains blocked until operator-provided device/evidence |
