...
- [ ] 30-minute physical Android terminal renderer dogfood passes the Section 7.6 termlib decision gate — local Android release artifact validation, debug emulator renderer smokes, direct adb pair/attach/input evidence, and the `yes | head -10000` emulator flood substitute keep development confidence high, but the original termlib decision gate still requires a physical Android device dogfood run that starts the daemon, renders a live `claude` session, types, scrolls, resizes, and pastes for 30 minutes before Play internal distribution.
