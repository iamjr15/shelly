...
- [x] Android AAB includes arm64-v8a, armeabi-v7a, x86_64 — latest local validation on 2026-05-23 rebuilt the release bundle against current Android source with `apps/android/gradlew --no-daemon bundleRelease` and passed `pnpm check:android-aab` against `apps/android/app/build/outputs/bundle/release/app-release.aab` (`54M`, SHA-256 `7a7868df2b8abd8e128b5ba46a40aa58bfac72b482c67e8c83351c4fb60b6ed0`). Earlier 2026-05-18 validation rebuilt `apps/android/scripts/build-rust.sh` and regenerated UniFFI Kotlin bindings for `arm64-v8a`, `armeabi-v7a`, and `x86_64`. The AAB verifier now also checks the packaged protobuf manifest uses-permission allowlist and privacy surface for required Firebase/Sentry opt-out metadata, rejects unwanted location, microphone, contacts, media, storage, session-name, command, and terminal-content fields, and enforces the local unsigned AAB state with `--expect-unsigned`; Android Studio's bundled `jarsigner` also reports `jar is unsigned`. Release bundle signing is still blocked by the separate release-keystore gate below.

### 13.3 Signing & distribution
- [ ] Daemon signed and notarized on macOS via rcodesign in CI — `release-rust.yml` signs Darwin `fieldworkd` with Developer ID, `--code-signature-flags runtime`, submits the zipped daemon to Apple notarization with `--wait --staple`, then runs `node scripts/verify-macos-signing.mjs` so the daemon must pass `codesign`, hardened-runtime, and Gatekeeper-notarized checks before archive staging. The gate remains unchecked until a real signed/notarized GitHub Release artifact is produced with operator-owned Apple credentials.
- [ ] iOS app signed with Apple Distribution cert, profiled with App Store profile
- [ ] Android AAB signed with release keystore (stored in GitHub Secrets, base64'd) — `release-android.yml` decodes the release keystore and Gradle signing properties from GitHub Secrets, builds `bundleRelease`, runs `node scripts/verify-android-aab.mjs --expect-signed` so the bundle must contain `META-INF` signature entries, and then runs `jarsigner -verify -certs` before Play internal-track upload. The gate remains unchecked until the real operator-owned release keystore signs the AAB in CI.
...
- [ ] Push notifications fire reliably for `AwaitingInput` state changes (10/10 in manual test) — local daemon push worker, relay payload/privacy validation, APNs/FCM provider-client error handling, token ownership, stale-token pruning, Android token registration, and Android notification tap routing are covered by unit/JVM/emulator smokes. `docs/ANDROID_FCM_PUSH.md` and `pnpm check:android-fcm-push-evidence -- "$FW_ANDROID_FCM_DIR"` now define the Android FCM evidence contract for 10/10 physical release-device `AwaitingInput` deliveries, inspected hash-only FCM HTTP v1 payloads, generic notification copy, and tap-through to the target daemon-owned session. This remains unchecked until real APNs/FCM provider delivery is exercised 10/10 on physical devices with relay-held provider credentials.
...
- [ ] Run 3 sessions in parallel, switch between them on phone, no state leakage — the hidden local handoff smoke verifies switched simulated-phone sessions do not receive each other's output markers, and `pnpm test:android-emulator-multisession` is the actual Android-app substitute: it opens `fwm_a`, `fwm_b`, and `fwm_c`, switches among all three in the app, sends Android-originated input to each, and verifies `multi_a_ok`, `multi_b_ok`, and `multi_c_ok` land only in their selected PTYs. `docs/ANDROID_MULTISESSION.md` and `pnpm check:android-multisession-evidence -- "$FW_ANDROID_MULTISESSION_DIR"` now define the physical release-device evidence contract for signed release builds: Android must switch among desktop-created `fwm_a`/`fwm_b`/`fwm_c`, each replay file must contain only its selected marker, and the verifier rejects cross-session marker leakage or mobile create/kill/command controls. Physical-device switching remains required before checking this gate.
...
`fieldworkd` files, copies root `LICENSE`/`NOTICE` into all five npm package
directories, and then runs `scripts/verify-npm-packages.mjs --require-binaries`
plus `scripts/publish-npm-packages.mjs --check-ready`. The script is a local
release-candidate helper only; `release-npm.yml` still publishes from verified
GitHub Release archives and Sigstore/SLSA attestations.
...
`BUILD_TYPE = "debug"`, `DEBUG = Boolean.parseBoolean("true")`,
`FIELDWORK_BIOMETRIC_BYPASS = false`, and
`FIELDWORK_DEBUG_PAIRING_PAYLOAD = ""`. `locked.png` is a 1080x2400 PNG with
