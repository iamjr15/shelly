...
Both binaries (`fieldwork` CLI + `fieldworkd` daemon) ship in the **same** per-platform tarball — each platform package contains a `bin/` directory with both binaries. The meta-package's `bin` field exposes both commands to npm so user service managers and direct daemon invocations resolve the same native package family as the CLI.
...
The corresponding **dispatchers** (`bin/fieldwork` and `bin/fieldworkd` as initially shipped — only run when postinstall couldn't swap) follow the Biome-minimal + Turbo-Ctrl-C pattern:
