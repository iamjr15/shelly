...
**Total upfront cost for desktop npm live testing/release**: $0. The Apple Developer Program is **not required** for the macOS npm CLI/daemon path; it is deferred until iOS/TestFlight resumes or until we choose to ship notarized Mac app/pkg artifacts.
...
- **npm meta-package**: `fieldwork` (with reserved per-platform packages `fieldwork-darwin-arm64`, `fieldwork-darwin-x64`, `fieldwork-linux-arm64`, and `fieldwork-linux-x64`)
...
**Architecture pillar**: iroh P2P (with self-hosted relay on Oracle ARM A1 Always Free) for zero-config NAT traversal, zero infrastructure cost, and zero ongoing operational burden. Because Oracle Mumbai A1 capacity is currently unavailable, v1 live testing may use an explicitly budgeted AWS Lightsail `relay` host as a paid fallback until Oracle capacity is secured; that does not change the production target or the relay privacy model.
...
- **Pairing code**: exactly 5 characters from the Crockford base32 alphabet (`0123456789ABCDEFGHJKMNPQRSTVWXYZ`, no `I`/`L`/`O`/`U`), ~25 bits, generated with `OsRng`. Case-insensitive input is normalized to uppercase (`I`/`L` → `1`, `O` → `0`). 10-minute TTL. A single code is active at a time; the daemon invalidates the active code after 5 wrong in-band attempts.
- **Pairing ticket**: the compact `PairingTicket { code, node_id, relay_url, addrs, expires_at }` carries both the daemon's reachability and the short code. It serializes to `fw1<base32>` (postcard → unpadded base32 behind the human prefix `fw1`); the QR encodes the whole ticket string. See Section 6.2 for the wire type.
- **Two pairing paths, one convergence**: a device pairs by EITHER scanning the QR OR typing the 5-character code. The QR carries reachability itself, so it works with no hosting today; the typed code resolves to the same reachability through the relay rendezvous endpoint, available once the relay is hosted. Both paths end identically — the phone dials the daemon over iroh, sends the code, the daemon verifies it (attempt-capped), and **desktop approval stays the final gate.**
...
- **Relay's iroh-relay function sees ciphertext only** (encrypted QUIC packets). Its HTTP control plane / push gateway sees push tokens, daemon NodeIDs/public keys, hashed session metadata, source IPs (scrubbed to /16), and — only on the typed-code path — short pairing codes mapped to opaque reachability blobs with a 10-minute TTL and per-code lockout. The QR path stays daemon-local. **No terminal content traverses the relay in either function.** See Section 7.3.2 for the full trust model split.
...
2. CLI generates a 5-character code, builds a `PairingTicket`, prints the encoded `fw1…` ticket as a QR, and also displays the bare 5-character code for manual entry. If `FIELDWORK_RELAY_CONTROL_URL` is set, the daemon best-effort publishes `code → ticket_blob` to the relay; if unset, it logs a warning and skips publish (QR still works, typed-code path is simply unavailable).
3. User opens Fieldwork on phone, then EITHER scans the QR (yields reachability + code, no typing) OR types the 5-character code (phone resolves it to the ticket via the relay rendezvous endpoint).
4. Phone's iroh endpoint connects to daemon's iroh endpoint (direct or via relay) using the ticket's reachability.
5. Phone sends `PairWithCode { code, device_name, device_node_id }`; the daemon normalizes and verifies the code (attempt-capped), the desktop prompts for explicit approval, and both sides exchange long-lived Ed25519 pubkeys.
6. Phone disconnects. Pair complete. The code expires (or is invalidated on first use / after the attempt cap).
...
| Crash reporting (daemon/mobile) | None in v1 | Relay Honeycomb only; daemon/mobile diagnostics are local opt-in preferences with no crash-reporting SDK |
...
| macOS npm trust prep | ad-hoc `codesign --sign -` + quarantine cleanup after integrity verification; optional `apple-codesign`/notarization only when Apple credentials exist | built-in macOS tools + latest `apple-codesign` for optional notarized artifacts |
...
| No crash-reporting SDK in v1 | No crash-reporting SDK in v1 |
...
| Live-test relay fallback | AWS Lightsail `relay` in Mumbai (`nano_3_1`), only while Oracle A1 capacity is blocked | ~$5/mo, guarded by a $10/mo Lightsail budget |
| GitHub Actions | Linux runners (free for OSS) | $0 |
| Honeycomb (relay traces) | Free tier (20M events/mo) | $0 |
| Apple Developer Program | Apple | $0 for desktop npm; $99/yr only when iOS/TestFlight resumes or when we intentionally add notarized Mac app/pkg artifacts |
| Google Play Developer | Google | $25 one-time |
| Domain `fieldwork.dev` | Namecheap or Cloudflare | ~$12/yr |
| **Total ongoing** | — | **Target: $0/mo infra; live-test fallback: ~$5/mo until Oracle A1 is available; $12/yr domain; Apple $99/yr deferred and not required for npm CLI/daemon live testing** |
...
│   │       ├── types.rs                # SessionId, ClientId, AgentState, Capabilities, PairingTicket
│   │       ├── code.rs                 # shared pairing-code alphabet + normalize/validate
│   │       └── version.rs              # CONTRACT_VERSION = 2
│   ├── daemon/
│   │   ├── Cargo.toml
│   │   └── src/
...
│   │       ├── pairing.rs              # pairing-code generation (OsRng), ticket build, relay publish, approval
...
│   │       ├── lib.rs                  # axum control plane: register node, push gateway, pairing-code publish/resolve rendezvous, aggregate metrics
│   │       ├── apns.rs                 # APNs provider client
│   │       └── fcm.rs                  # FCM provider client
│   └── mobile-core/
...
│   │   ├── watch-a1-capacity.sh        # capacity-report poller before apply
...
- **`CONTRACT_VERSION = 2` constant in `protocol` crate** (bumped from 1 for the breaking pairing wire change — acceptable pre-release). Server rejects `Hello` with mismatch; direct IPC tests cover `LocalCli`, `IosApp`, and `AndroidApp`, and the local iroh handoff smoke verifies mobile transport mismatch rejection before pairing.
...
pub enum ClientToServerMsg {
    ...
    Ping { seq: u64 },
    // Pairing administration is LocalCli-only. Remote devices use PairWithCode
    ...
    // Presents the short pairing code (from a QR scan or manual entry). The
    // daemon normalizes and verifies it against the single active code,
    // invalidating that code after 5 wrong attempts.
    PairWithCode {
        code: String,
        device_name: String,
...
pub enum ServerToClientMsg {
    ...
    PairingStarted { ticket: PairingTicket },
...
// Compact pairing target carrying both reachability and the short code. The QR
// encodes the whole ticket (scanning yields BOTH reachability AND the code — no
// typing); the typed-code path resolves the code to this same ticket via the
// relay rendezvous endpoint. Field ORDER is load-bearing — postcard is
// positional (no field names on the wire).
#[derive(Serialize, Deserialize, Clone, Debug, Eq, PartialEq)]
pub struct PairingTicket {
    pub code: String,                  // the 5-char Crockford code (the only credential)
    pub node_id: String,               // daemon iroh node id
    pub relay_url: Option<String>,     // daemon's advertised iroh relay url, when available
    pub addrs: Vec<String>,            // direct socket addresses for local-network dials
    pub expires_at: u64,               // ms; 10-minute TTL
}

impl PairingTicket {
    // Encodes as `fw1<base32>`: postcard bytes wrapped in unpadded base32 behind
    // the human prefix. decode() is case-insensitive on the body, trims
    // whitespace, and requires the exact `fw1` prefix.
    pub fn encode(&self) -> Result<String, TicketError>;
    pub fn decode(s: &str) -> Result<PairingTicket, TicketError>;
}
...
- **Unit test gates** (in Section 13.7): deterministic terminal-model coverage asserts synthetic ANSI snapshots rehydrate visible cells, attributes, and cursor exactly. The real-PTY gate starts `vim /etc/hosts`, forces stale attach, feeds the synthetic `Attached.initial_bytes` into a fresh in-process `wezterm-term` client, and asserts the client reaches the same alt-screen mode, cursor position, and rendered visible text as the daemon model. Without both gates passing, v1.0 does not ship.
...
| **Daemon/mobile diagnostics** | Local opt-in diagnostics preference only. No daemon/mobile crash-reporting SDK and no daemon OTLP/Honeycomb export are bundled in v1. | NO terminal/session content by design. | **Opt-in only; user defaults to off.** No daemon/mobile diagnostic data leaves the user's machine in v1. |
...
- Detect session state (`Idle | Working | AwaitingInput | Crashed`) via per-agent inference modules. **v1 ships two**:
  ...
  - **`state_infer::codex`** — uses Codex structured state events when available, accepting `type`, `event`, and `status` event shapes. Implementation note from the local Codex CLI surface (2026-05-23, `codex-cli 0.133.0`): the installed command exposes `codex remote-control start`, `codex app-server --listen/proxy`, and `codex app-server daemon {start,enable-remote-control,...}`, but `codex` itself still does not accept a `--remote-control` flag and the older planned `codex app-server daemon --remote-control` flag does not exist. v1 must not mutate a user's `codex` PTY command into an unsupported flag because universal terminal handoff takes priority. Instead, Fieldwork runs the requested `codex` PTY command unchanged and accepts structured Codex JSON or JSONL events through the local agent-event hook adapter; app-server/proxy integration can be layered onto that adapter once the command surface is stable. Matches commands `codex`, `codex-exec`.
...
```
fw pair                                 # show QR + 5-char code for new device; prompts to approve incoming pair requests
fieldwork pair                          # full command name for the same scan-QR-or-type-code flow
fieldwork pair-test --payload <fw1-ticket> [--attach <session|first>]
fieldwork pair-test --code <code> --relay-control-url <url> [--attach <session|first>]
                                        # hidden headless iroh transport smoke client (ticket or relay-resolved code)
...
fw daemon                               # subcommand group
  ...
  fw daemon logs [--tail]
  fw daemon restart
fw doctor [--no-start]                  # check CLI, colocated daemon binary, socket hardening/protocol, sessions, and privacy settings
fieldwork settings telemetry status
fieldwork settings telemetry on
...
### 7.3 `fieldwork-relay` (Oracle ARM target, AWS live-test fallback)
...
2. **axum HTTP control plane / push gateway** on `:8443`, served as HTTPS in production by the Rust process with a relay-only `control-plane.crt`/`control-plane.key` loaded through systemd credentials. Local development may run the same listener as loopback HTTP when `FIELDWORK_RELAY_REQUIRE_TLS` is unset.
   - `POST /v1/pair` — register the daemon's relay-signing public key for signed push requests. The QR pairing path remains daemon-local and is not stored by the relay.
   - `POST /v1/pair/publish` — **pairing rendezvous (publish)**. Signed by the daemon (`X-Fieldwork-Signature` over `(method, path, body, nonce, ts_ms)`, same scheme as push). Body `{daemon_node_id, code, ticket_blob, expires_at_ms, nonce, ts_ms}`, where `ticket_blob` is the opaque encoded `fw1…` ticket string. Stores `code → ticket_blob` with a 10-minute TTL (rejects already-expired publishes). A daemon advertises a single active code; publishing supersedes any prior code for that daemon. Best-effort — the daemon only calls this when `FIELDWORK_RELAY_CONTROL_URL` is set.
   - `GET /v1/pair/resolve/{code}` — **pairing rendezvous (resolve)**. Unauthenticated (the code is the credential), but defended: per-client rate limit (20 attempts/min), uniform `404` whether the format or the lookup failed, the code is consumed on a correct guess, and a code is locked out after 5 wrong resolves. Returns `{ticket_blob}` on a hit so the phone can decode the ticket and dial the daemon. **The blob is opaque reachability only — no terminal content ever touches the relay.**
...
| **HTTP control plane / push gateway** | Push tokens (registered by phones, sent to APNs/FCM by relay). Hashed session IDs/names. Daemon NodeIDs and relay-signing public keys. Source IP of every request. Operational logs at info level (TTL'd). On the typed-code path only: short pairing codes mapped to opaque reachability blobs (the encoded `fw1…` ticket), held with a 10-minute TTL and per-code lockout. | Terminal content. Command lines. File paths. Session names in plaintext (hashed). User identity beyond NodeID. The QR pairing path stays daemon-local. The pairing blob is opaque reachability only — never terminal content. |
...
- **Live-test fallback while Oracle A1 is out of capacity**: AWS Lightsail instance `relay` in `ap-south-1a`, static IP `3.7.208.153`, bundle `nano_3_1`, ports `80/tcp`, `443/tcp`, `8443/tcp`, and `7842/udp` open, SSH restricted to the current operator IP, and AWS Budgets alerting at 80%, 100%, and forecasted 100% of a $10/month Lightsail budget. The deployed service is an IP-only HTTP control-plane candidate at `http://3.7.208.153:8443` for local/live typed-code testing until operator-owned DNS/TLS and full provider credentials are installed. `pnpm test:hosted-relay` is the explicit external smoke for this bridge: it starts an isolated local daemon, publishes a pairing code to the hosted relay, resolves it through `fieldwork pair-test --code`, requires desktop approval, attaches to a daemon-owned PTY, sends input, and verifies `HOSTED_RELAY_RESULT_OK`. A hosted run passed on 2026-05-29 against `http://3.7.208.153:8443`. As of 2026-05-29 it has a relay-only FCM service-account key installed at `/etc/fieldwork/secrets/fcm-service-account.json` with `0440 root:fieldwork-relay` permissions and a systemd drop-in setting `FIELDWORK_FCM_SERVICE_ACCOUNT_PATH`; APNs, Honeycomb, DNS/TLS, iroh fallback proof, and physical-device provider verification remain outside this bridge sign-off. It is not the final production HTTPS relay sign-off.
...
impl FieldworkClient {
    ...
    // QR path: decode the scanned `fw1…` ticket locally (no hosting needed).
    pub async fn pair_with_qr(self: Arc<Self>, ticket: String) -> Result<DaemonInfo, FieldworkError>;
    // Typed-code path: resolve the 5-char code to a ticket via the relay
    // rendezvous (requires ClientConfig.relay_control_url to be set), then dial.
    pub async fn pair_with_code(self: Arc<Self>, code: String) -> Result<DaemonInfo, FieldworkError>;
...
- **Security-sensitive parser/auth coverage**: v1 uses focused Rust unit tests, proptests, protocol snapshots, authz tests, and the local handoff smoke for pairing-code normalize/validate, `PairingTicket` encode/decode round-trips, device auth, revocation, and mobile capability rejection. Bounded model-checker harnesses are not a v1 release gate.
...
3. **Pairing** (`Features/Pairing/PairingView.swift`):
   - Camera QR scanner (`AVCaptureSession`) for scanning the `fw1…` ticket, plus a manual 5-character code field.
   - On scan, decodes the ticket directly. On manual entry, resolves the code to a ticket via the relay rendezvous endpoint (`FIELDWORK_RELAY_CONTROL_URL`, injected through the Info.plist key `FieldworkRelayControlURL` into `ClientConfig.relay_control_url`). Either way, calls mobile-core to dial the daemon and present the code.
...
- Unpair button
- App version + diagnostics sharing toggle
- About / OSS licenses
...
- **Pairing requires desktop confirmation**: when the phone dials in and sends `PairWithCode`, the daemon does not auto-approve. Desktop CLI prints `Pair request from device "{name}" — approve? [y/N]` and waits for explicit `y`. The 5-character code is the only credential carried over the wire (whether the phone got it from a QR scan or manual entry); the daemon verifies it attempt-capped and invalidates the active code after 5 wrong attempts. Desktop approval remains the terminal authority regardless of how the code arrived.
...
function prepareMacBinary(file) {
  ...
  // Runs only after npm has verified package integrity and we have copied the
  // platform package binary into the meta-package bin dir.
  bestEffort('codesign', ['--force', '--sign', '-', file]);
  bestEffort('xattr', ['-d', 'com.apple.quarantine', file]);
}
...
try {
  ...
  prepareMacBinary(path.join(binDir, 'fieldwork'));
  prepareMacBinary(path.join(binDir, 'fieldworkd'));
} catch (err) {
...
The real implementation also verifies the release manifest for the selected
platform package before the macOS `codesign`/`xattr` steps. Removing
`com.apple.quarantine` is allowed only for the two verified Fieldwork
executables, never recursively across `node_modules`.
...
## 9. Code signing strategy (npm-first, no paid macOS gate)

Fieldwork's desktop distribution is a developer CLI installed through npm, not a
Mac app, `.pkg`, `.dmg`, Homebrew cask, or curl installer. v1 therefore follows
the esbuild/SWC/Deno-style native npm package model: npm package integrity,
repo-owned checksums/provenance, executable-bit repair, local ad-hoc signing,
and targeted quarantine cleanup. **Apple Developer ID notarization is not a
desktop npm release blocker.**

Research decision (2026-05-26): CodexBar, GitHub CLI, Node, Homebrew, and
Electron-style apps use Developer ID signing/notarization for polished Mac apps
or installers. Standalone developer CLI tarballs and npm-native packages commonly
ship without notarizing each raw binary. Fieldwork uses that second model for
desktop v1. The plan must not require the operator to buy Apple Developer
Program membership just to live-test or release the npm CLI/daemon.

| Component | Required for v1 npm path? | Cost | Tool |
|---|---|---|---|
| **CLI on macOS via npm** | Yes: executable bit, ad-hoc Mach-O signature, quarantine cleanup after integrity verification | $0 | `chmod`, `codesign --force --sign -`, `xattr -d com.apple.quarantine` |
| **Daemon on macOS via npm** | Yes: same as CLI; `fieldwork daemon install` must preflight that `launchd` can execute the installed `fieldworkd` | $0 | `codesign --force --sign -`, `launchctl` preflight, `fw doctor` |
| **CLI/daemon on Linux via npm** | Executable bit + checksum/provenance verification only | $0 | `chmod`, SHA-256/Sigstore/npm provenance checks |
| **CLI on Windows** | N/A for v1 — document `WSL2 + linux-x64 build`; native Windows host is tracked in `FUTURE.md`. | — | — |
| **Developer ID notarized Mac artifacts** | Optional/deferred. Only run when Apple secrets exist; do not block npm desktop live testing or npm release. | $99/yr only if adopted | `apple-codesign`/`notarytool` |
| **iOS app** | Mandatory only when iOS/TestFlight/App Store work resumes | $99/yr Apple Developer | Xcode / Fastlane |
| **Android app** | Mandatory for Play release (self-signed upload key / Play app signing) | $0 + $25 one-time Play | Gradle signingConfig |
| **GitHub Release tarballs (any)** | Audit artifacts, not install channel | $0 | SHA-256 + Sigstore/SLSA attestations |

**macOS npm trust rules:**
- Run no live network download from `postinstall`; platform binaries come from npm
  optional dependencies.
- Verify npm package integrity and the Fieldwork release manifest before
  modifying copied binaries.
- Apply `chmod 0755` to `fieldwork` and `fieldworkd`.
- On Darwin only, run `codesign --force --sign -` on both copied binaries. This
  creates a local ad-hoc signature and does not require Apple credentials.
- Remove `com.apple.quarantine` only from the two verified Fieldwork
  executables. Never recursively strip xattrs across `node_modules`.
- `fieldwork doctor` must report the current trust mode:
  `npm/ad-hoc/not-notarized` or `Developer ID/notarized`.
- `fieldwork daemon install` must refuse to install/start a daemon that fails the
  local execute/launchd preflight, and must print the exact repair command.
- `fieldwork daemon install` must persist a narrow non-secret service
  environment, including `PATH`, `HOME`, `XDG_RUNTIME_DIR`, XDG config/state
  paths, and non-secret Fieldwork runtime flags, so launchd/systemd daemons can
  find user-installed commands like `claude`, `codex`, `node`, `python`, and TUI
  tools while agreeing with the CLI on the control-socket path. It must not
  persist key material such as iroh identity overrides or relay signing keys.

**Optional Developer ID path:**

Keep the `apple-codesign`/notarization scaffolding in CI, but make it conditional
on Apple credentials. When `APPLE_P12_BASE64`, `APPLE_P12_PASSWORD`, and App
Store Connect API credentials are present, release jobs may additionally produce
notarized Darwin audit artifacts. When they are absent, the desktop npm release
continues with ad-hoc-signed Darwin binaries and records "notarization skipped:
Apple credentials absent" in `docs/RELEASE_AUDIT.md`.
...
| `version-packages.yml` | Push to main + manual dispatch | Verify the Changesets fixed group, then run `changesets/action@v1` with pinned `pnpm dlx` Changesets packages to open the version-packages PR without creating a mutable root install. Publishing is intentionally absent here; `release-npm.yml` owns npm provenance publish after verified Rust artifacts exist. |
| `release-rust.yml` | Tag `v*.*.*` (Changesets-managed) | Cross-compile matrix (4 host targets: darwin-arm64, darwin-x64, linux-x64, linux-arm64; native runners + cargo-zigbuild for Linux), verify provider secret boundaries and telemetry privacy wiring, ad-hoc sign Darwin CLI/daemon binaries on macOS runners, generate SHA-256 + Sigstore/SLSA audit artifacts, and upload artifacts to GitHub Releases for audit. Apple Developer ID signing/notarization is conditional and additive when Apple credentials exist; missing Apple credentials must not fail the desktop npm release path. **cargo-dist is used only for the build/archive pipeline — its installer/brew-formula generators are explicitly disabled** (we ship via npm only). |
...
| `release-android.yml` | Tag `android-v*.*.*` | ubuntu-24 runner: fail closed before toolchain setup and Rust/mobile build when Firebase, signing, HTTPS relay-control URL, or Play upload secrets are absent; cargo-ndk matrix per ABI, generate Kotlin bindings, verify telemetry privacy wiring, assembleRelease with `FIELDWORK_RELAY_CONTROL_URL` injected for typed-code pairing, verify mobile privacy defaults and store privacy answer sheet, verify AAB ABI contents plus packaged manifest privacy surface, release relay URL, and signature, upload AAB to Play Console internal track. |
...
- **Crashes**: no daemon crash-reporting SDK is bundled in v1.
- **Opt-in toggle**: `fieldwork settings telemetry on` persists local daemon diagnostics consent in `config.toml`. v1 does not export daemon diagnostics remotely; daemon OTLP/Honeycomb export is intentionally absent and the telemetry privacy verifier rejects accidental daemon OTLP/Honeycomb wiring.
- **Consent flow on mobile**: **delayed opt-in**. No consent prompt on first launch (would block the wow-moment of pairing). After the user's first session crosses `AwaitingInput → user responds → agent emits 10+ lines of output` (i.e., they've experienced the value), surface a one-time bottom-sheet prompt: *"Help improve Fieldwork? Product diagnostics only. No code, prompts, terminal output, or file paths."* Two buttons: `Sure` / `No thanks`. Decline = silent and final; Settings remains available for later changes. v1 stores this as a local preference and does not start a mobile crash-reporting SDK.
...
- **Logs**: stderr → journalctl. Log level `info` and above ship to a free-tier log aggregator (Better Stack or Grafana Loki self-hosted on the same A1 instance). Push tokens, pairing codes, and NodeIDs are explicitly redacted via a `tracing::Layer` sanitizer.

Privacy posture documented in `docs/PRIVACY.md`; self-hostable relay packaging is outside v1 and tracked in `FUTURE.md`.

### 11.3 Mobile diagnostics consent

- No iOS or Android crash-reporting SDK is bundled in v1.
- User-toggleable diagnostics sharing exists in app Settings and through the delayed one-time consent prompt; off by default.
- The mobile preference remains local in v1. Any future hosted mobile crash reporting requires a plan update, privacy-doc update, store-disclosure update, and release-gate update before implementation.
...
- [x] CLI builds clean on macOS arm64, macOS x86_64, Linux x86_64, Linux arm64 (4 v1 targets; native Windows host is outside v1) — latest local release build pass on 2026-05-20 produced executable `fieldwork` binaries for all four targets.
- [x] Daemon builds clean on same 4 targets — latest local release build pass on 2026-05-20 produced executable `fieldworkd` binaries for all four targets.
- [x] Relay builds for Linux arm64 (Oracle ARM target) — latest local release build pass on 2026-05-20 also produced `fieldwork-relay` for both Linux targets and both Darwin targets.
- [ ] iOS xcframework includes arm64-device, arm64-sim, x86_64-sim — current local Mac has macOS 15.2 and only Command Line Tools selected. Apple's compatibility table makes Xcode 16.3 the newest viable full Xcode for this host; Xcode 16.4 requires macOS 15.3+ and Xcode 26.x requires macOS 15.6+/26.x. Apple App Store Connect uploads now require Xcode 26+ with an iOS 26+ SDK, so `release-ios.yml` uses a `macos-26` runner and `scripts/check-ios-prereqs.sh --release` verifies Xcode/iOS SDK major versions before TestFlight upload. Local prerequisites that do not require Apple credentials are installed/downloaded: `xcodes` 1.6.2, `aria2` 1.37.0_2, `.xcode-version` pins local Xcode `16.3`, the required Rust iOS targets are installed, and the local reference cache has `SwiftTerm` `v1.13.0` and `blink`. `xcodes update --data-source xcodeReleases` confirms Xcode `16.3 (16E140)` and Xcode 26.x release availability. Generated `target/debug` and Android build intermediates were cleaned while preserving the release AAB, and the latest local audit reports at least 70 GiB free in `~/Downloads`, satisfying the repo script's Xcode download/expansion guard. `apps/ios/scripts/build-rust.sh` now runs `scripts/check-ios-prereqs.sh` before Cargo/Xcode work and switches to `--release` mode when the release-runner Xcode/SDK floor environment is present. When full Xcode is missing, `scripts/check-ios-prereqs.sh` prints concrete recovery steps to authenticate, run `scripts/check-ios-prereqs.sh --download-xcode`, expand or place `Xcode_16.3.xip`, select `/Applications/Xcode-16.3.app/Contents/Developer`, run `sudo xcodebuild -runFirstLaunch`, rerun `pnpm check:ios-prereqs`, and then run `apps/ios/scripts/build-rust.sh`. `scripts/check-ios-prereqs.sh --download-xcode` and direct `xcodes download 16.3 --data-source xcodeReleases` report a missing Apple ID/password or require an authenticated Apple Developer session, direct `curl` against Apple's XIP URL redirects to the unauthorized page, and the existing Chrome session remains blocked by Apple Developer authentication/access; no Xcode `.xip` is present in `~/Downloads`. Direct `fieldwork-mobile-core` iOS target builds now fail at that prereq check because `xcrun --sdk iphoneos`/`iphonesimulator` cannot locate the SDKs without full Xcode selected.
- [x] Android AAB includes arm64-v8a, armeabi-v7a, x86_64 — latest local validation on 2026-05-30 rebuilt the release bundle against current Android source with `apps/android/gradlew --no-daemon :app:bundleRelease` and passed `pnpm check:android-aab` against `apps/android/app/build/outputs/bundle/release/app-release.aab` (`57M`, SHA-256 `af38adfb7541caf31c45afa216c61c4fa2dbce9ab1168ce91181f91a1f0ccca8`). Earlier 2026-05-18 validation rebuilt `apps/android/scripts/build-rust.sh` and regenerated UniFFI Kotlin bindings for `arm64-v8a`, `armeabi-v7a`, and `x86_64`. The AAB verifier now also checks the packaged protobuf manifest identity, version, uses-permission allowlist, and privacy surface for required Firebase opt-out metadata; verifies the release `BuildConfig` targets `app.fieldwork.android`, `versionCode=1`, `versionName=1.0`, `BUILD_TYPE=release`, `DEBUG=false`, biometric bypass off, and empty debug pairing code; rejects unwanted location, microphone, contacts, media, storage, debuggable, session-name, command, terminal-content fields, and Sentry/crash-reporting SDK markers in archive entry names or packaged dex/manifest content; enforces the local unsigned AAB state with `--expect-unsigned`; and enforces an HTTPS `FIELDWORK_RELAY_CONTROL_URL` only when the signed release path passes `--expect-relay-control-url`. `pnpm test:android-aab-signing-smoke` signs a temporary copy of the current real AAB with an ephemeral non-debug certificate and verifies that copy through the same `--expect-signed` verifier without modifying the retained unsigned local bundle. Android Studio's bundled `jarsigner` also reports `jar is unsigned`. A 2026-05-30 local release APK install smoke converted the current AAB with `bundletool-all-1.18.3` into `/tmp/fieldwork-android-release-install-20260530045350/apks/fieldwork-release-universal.apks`, extracted `universal.apk`, verified APK Signature Scheme v3 with an ephemeral non-debug `CN=Fieldwork Release Smoke` signer, confirmed `package: name='app.fieldwork.android' versionCode='1' versionName='1.0'`, found no `debuggable` marker, installed it on `Medium_Phone_API_36.1`, and launched the real release package with `Status: ok`, `LaunchState: COLD`, retained passing launch attempt `TotalTime=1169ms`, and a locked `Unlock` UI under `/tmp/fieldwork-android-release-install-20260530045350`; `launch-attempts.txt` records earlier cold-start variance, `run-as` reported `package not debuggable`, `package-info.txt` omitted `DEBUGGABLE`, and the crash buffer was empty. This is still local ephemeral-signing/emulator evidence only; release bundle signing is blocked by the separate release-keystore gate below.
  Android release readiness command (2026-05-25, tightened 2026-05-30): `scripts/check-android-release-readiness.mjs` and `pnpm check:android-release-readiness:local` now consolidate the desktop command-surface checks used during capture, Android release AAB/content verifier, release `BuildConfig`, mobile/store privacy verifiers, release workflow verifier, release-signing/install evidence contracts, release-secret presence, AAB signing state, and physical release-phone install preflight into one command. Local mode accepts the installed `fw`/`fieldworkd` commands when present and otherwise falls back to an internal temporary `fw`/`fieldwork`/`fieldworkd` shim backed by repo-local `target/release/fieldwork` plus `target/release/fieldworkd`; the fallback must prove alias-aware `Usage: fw`, `Usage: fw doctor`, and `Usage: fieldworkd`, while still reporting the unsigned AAB, missing release secrets, missing HTTPS `FIELDWORK_RELAY_CONTROL_URL` for Android typed-code pairing, and absent physical release phone as pending. Strict mode fails until current `fw` and `fieldworkd` commands, the real signed AAB, required release secrets, exactly one authorized physical phone, and non-debuggable installed `app.fieldwork.android` package are available. `pnpm test:android-release-readiness` covers the pending-vs-strict behavior without credentials or a device.
  Android release evidence pack scaffold (2026-05-25): `scripts/create-android-release-evidence-pack.mjs` and `pnpm scaffold:android-release-evidence-pack -- --print-dir` now create one top-level signed-release evidence workspace with a source-checkout `fw`/`fieldwork`/`fieldworkd` command shim plus focused subdirectories for release signing, release install, pair flow, session subscription, terminal attach, resize/detach, biometric, dogfood, cold start, renderer flood, background/foreground, network reconnect, restart restore, multisession, and FCM push. The pack writes `setup.sh`, `manifest.json`, `capture-order.md`, `readiness.sh`, and `verify.sh`; `readiness.sh` puts the generated command shim on `PATH`, runs local Android release readiness, then runs `fw doctor` to prove the desktop CLI can auto-start and handshake with `fieldworkd` before physical evidence capture. `verify.sh` runs every focused Android release evidence verifier in capture order, including the strict release-install physical-device check. It delegates to the focused scaffolds and explicitly does not fabricate verifier evidence. `scripts/test-android-release-evidence-pack.mjs` plus `pnpm check:local-release` keep the pack wired to every focused Android release-device evidence scaffold.

### 13.3 Signing & distribution
- [ ] macOS npm CLI/daemon trust handling verified without Apple credentials — Darwin platform packages must ship `fieldwork` and `fieldworkd` with executable bits, npm/SHA-256/Sigstore provenance, ad-hoc Mach-O signatures, and no `com.apple.quarantine` xattr on the two installed executables after postinstall. `fieldwork doctor` must report `npm/ad-hoc/not-notarized`, and `fieldwork daemon install` must prove the installed `fieldworkd` can pass the local execute/launchd preflight before installing or starting a service. This replaces the previous Developer ID notarization release gate for the npm desktop path.
  Local implementation update (2026-05-29): `packages/cli/install.js` now runs Darwin-only `codesign --force --sign -` and targeted `xattr -d com.apple.quarantine` after copying `fieldwork` and `fieldworkd`; `fieldwork doctor` now reports the macOS trust mode before daemon connection checks. `scripts/smoke-npm-local-install.mjs` packs the local host platform package plus unscoped `fieldwork` meta package, installs both into a clean temp npm project, verifies `fieldwork`/`fw`/`fieldworkd` command surfaces, rejects leftover JS dispatchers after postinstall, and runs the macOS npm trust verifier on the installed Darwin binaries. `scripts/smoke-macos-daemon-launchd.sh` installs those npm-trust-prepared binaries as a temporary user LaunchAgent, verifies `fw doctor --no-start`, proves launchd and the CLI agree on the temp `XDG_RUNTIME_DIR` socket, kills the test `fieldworkd`, verifies launchd restores socket reachability within 10 seconds, confirms the restored session list and persisted scrollback replay remain available from a temp project directory outside macOS Desktop/Documents TCC-protected locations, and uninstalls the service. Local verification passed with `pnpm test:npm-dispatcher`, `pnpm test:npm-local-install`, `pnpm test:macos-daemon-launchd`, `pnpm check:npm-packages`, `cargo test -p fieldwork-cli doctor_`, `pnpm test:cli-doctor`, `pnpm check:macos-signing -- target/release`, `cargo clippy -p fieldwork-cli -- -D warnings`, `pnpm test:macos-signing`, `pnpm check:daemon-service`, and the docs/release-audit checks. Latest retained launchd smoke on 2026-05-30 kept `/tmp/fwld.i7Ckgt/evidence`, showed `npm/ad-hoc/not-notarized` trust in `fw doctor --no-start`, restored socket reachability after `pkill -KILL fieldworkd` in `restart_ms=369`, replayed `MACOS_KILL_SCROLLBACK_BEFORE` in both `kill-live-replay.txt` and restored `kill-replay.txt`, reported `[fieldwork: session exited 0]` after restart, and captured a clean daemon log; `pnpm check:macos-daemon-survival-evidence -- /tmp/fwld.i7Ckgt/evidence` still fails only because `sleep-wake.txt` and `sleep-replay.txt` were not captured. The release gate remains unchecked until real npm/release-artifact evidence captures package identity, provenance, installed-package doctor output, and retained launchd preflight evidence.
  macOS signing evidence contract (revised 2026-05-26, tightened 2026-05-30): `scripts/create-macos-signing-evidence-dir.mjs` must scaffold evidence for the zero-dollar npm path first and `scripts/verify-macos-signing-evidence.mjs` must verify it: installed package identity, per-Darwin-package checksum/npm integrity and npm/Sigstore provenance verification for `fieldwork-darwin-arm64` and `fieldwork-darwin-x64`, `codesign --display --verbose=4` showing ad-hoc signatures, `xattr -p com.apple.quarantine` proving absence on installed `fieldwork`/`fieldworkd`, `fw doctor` trust-mode output, and a local `launchctl`/daemon preflight, without signing binaries or running GitHub workflows. Developer ID authority, TeamIdentifier, hardened runtime, Gatekeeper notarization, and `stapler` evidence are optional additive evidence only when Apple credentials exist; their absence must not fail desktop npm live testing or npm release. Run `pnpm check:macos-signing-evidence -- "$FW_MACOS_SIGNING_DIR"` after capturing the evidence.
- [ ] iOS app signed with Apple Distribution cert, profiled with App Store profile
- [ ] Android AAB signed with release keystore (stored in GitHub Secrets, base64'd) — `release-android.yml` decodes the release keystore and Gradle signing properties from GitHub Secrets, injects the HTTPS `FIELDWORK_RELAY_CONTROL_URL` into `bundleRelease`, runs `node scripts/verify-android-aab.mjs --expect-signed --expect-relay-control-url` so the bundle must contain `META-INF` signature entries, pass `jarsigner -verify -certs`, report `jar verified`, include an HTTPS relay-control URL for typed-code pairing, and not use the Android Debug certificate, then runs the same `jarsigner` command again before Play internal-track upload. The local ephemeral signing smoke proves the current AAB and verifier accept a real non-debug JAR signature on a throwaway copy, but this gate remains unchecked until the real operator-owned release keystore signs the AAB in CI. `scripts/verify-android-release-signing-evidence.mjs` defines the signed-release evidence contract for `artifact-signing.txt`, `jarsigner.txt`, `sha256.txt`, `buildconfig.txt`, and `workflow-run.txt`; `scripts/test-android-release-signing-evidence.mjs` covers verifier failures for unsigned/debug/local-smoke evidence and non-HTTPS relay URL evidence; and `scripts/create-android-release-signing-evidence-dir.mjs` plus `scripts/test-android-release-signing-scaffold.mjs` provide the capture scaffold without fabricating evidence.
...
- [x] Daemon/mobile crash-reporting SDKs are absent — `pnpm check:telemetry-privacy`, `pnpm check:mobile-privacy`, the Android APK/AAB artifact verifiers, and the focused mobile telemetry tests verify default-off local diagnostics consent, the delayed mobile consent trigger, no daemon OTLP/Honeycomb export, no mobile crash SDK initialization path in v1, and no packaged Sentry/crash-reporting SDK markers in Android artifacts.
- [ ] Honeycomb receives test traces from relay — relay OTLP/Honeycomb export is wired with 1% default sampling, aggregate/static fields only, relay-only credential loading, and local loopback coverage via `pnpm test:relay-otlp`; `docs/RELAY_HONEYCOMB.md`, `scripts/verify-relay-honeycomb-evidence.mjs`, and `scripts/create-relay-honeycomb-evidence-dir.mjs` now define the hosted evidence contract and scaffold for relay config, systemd credential proof, `/v1/version` test traffic, Honeycomb query export, and redacted relay logs. The live Honeycomb receipt gate remains unchecked until a Honeycomb account/API key and hosted relay test traces are available.
- [ ] Daemon survives `pkill -KILL fieldworkd` and restarts via launchd/systemd — local unit coverage now verifies the rendered LaunchAgent uses `KeepAlive` with `SuccessfulExit=false`, `LimitLoadToSessionType=Aqua`, and non-secret `EnvironmentVariables`, including `XDG_RUNTIME_DIR`; `fieldworkd` reserves stdio with `/dev/null` before PTY creation so launchd cannot hand `openpty()` fd 0/1/2; the rendered systemd user unit uses `Restart=on-failure`/`RestartSec=5` plus non-secret `Environment="PATH=..."` and `Environment="XDG_RUNTIME_DIR=..."` through fake `launchctl`/`systemctl`; service install preserves the command-search and control-socket environment needed for arbitrary user CLIs without persisting iroh/relay signing secrets; and a fresh service install is rolled back if service start fails. `pnpm test:macos-daemon-launchd` now locally proves npm-trust-prepared Darwin binaries can install as a temporary user LaunchAgent, pass `fw doctor --no-start`, restart after `pkill -KILL fieldworkd` in `restart_ms<=10000`, and restore the session list plus persisted scrollback from a temp project directory outside macOS Desktop/Documents TCC-protected locations. The 2026-05-30 retained smoke at `/tmp/fwld.i7Ckgt/evidence` restored socket reachability in `restart_ms=369`; `kill-live-replay.txt` and `kill-replay.txt` both contain `MACOS_KILL_SCROLLBACK_BEFORE`; restored attach reported `[fieldwork: session exited 0]`; and the daemon log was clean. `docs/MACOS_DAEMON_SURVIVAL.md` and `pnpm check:macos-daemon-survival-evidence -- "$FW_MACOS_DIR"` still define the stricter npm-installed/ad-hoc-signed artifact evidence contract for a launchd-managed `pkill -KILL fieldworkd` restart with restored terminal scrollback; `kill-restart.txt` must document `processes_died_documented` because live PTY child processes are not expected to survive daemon death. The real survival gate remains unchecked until that retained evidence is captured on a verified npm-installed Darwin daemon or an actual Linux user-service host; Developer ID notarization is optional evidence, not the gate.
- [ ] Daemon survives `sleep 30 && wake` on macOS (lid close) — daemon install/restart and service-manager contracts are locally verified, and the macOS daemon survival evidence verifier now requires `sleep_duration_ms>=30000`, post-wake socket reachability, and restored terminal scrollback from the npm-installed/ad-hoc-signed launchd service. Real macOS sleep/wake survival remains unchecked until it can be run against the verified npm-installed daemon on a Mac with launchd managing the installed service.
- [ ] iOS app survives `Background → Foreground` with active session (reconnects)
- [ ] Android app survives same — `pnpm test:android-emulator-background-replay` is the local debug-app substitute: it pairs the actual Android app to an isolated release daemon, opens a desktop-created terminal, sends input before backgrounding, backgrounds the app while the PTY emits `ANDROID_BACKGROUND_REPLAY_OUTPUT`, foregrounds back to the attached terminal, sends `after_background_ok`, and uses a separately approved verifier to confirm the background-emitted output plus post-foreground input remain replayable. `docs/ANDROID_BACKGROUND_FOREGROUND.md` and `pnpm check:android-background-foreground-evidence -- "$FW_ANDROID_BG_DIR"` now define the physical release-device evidence contract for signed release artifact, normal biometric-gated app resume, replay of background-emitted PTY output, post-foreground Android-originated input, no Android fatal/ANR logcat entries, no Android system not-responding overlays, and empty crash buffers. Latest local run on 2026-05-19 passed on `emulator-5554`. Physical release-device background/foreground evidence remains required before checking this gate.
  Android background/foreground evidence scaffold note (2026-05-24): `scripts/create-android-background-foreground-evidence-dir.mjs` now creates the Android release-device background/foreground evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, installed package identity/version, before-background UI, KEYCODE_HOME background-state proof, after-foreground UI, logcat, and crash buffer. It still requires real `fw attach` replay transcripts and human-reviewed timing before verification; `scripts/test-android-background-foreground-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator/debuggable evidence, and does not fabricate PTY replay or timing files.
- [ ] Push notifications fire reliably for `AwaitingInput` state changes (10/10 in manual test) — local daemon push worker, relay payload/privacy validation, APNs/FCM provider-client error handling, token ownership, stale-token pruning, Android token registration, and Android notification tap routing are covered by unit/JVM/emulator smokes. `docs/ANDROID_FCM_PUSH.md`, `scripts/verify-android-fcm-push-evidence.mjs`, and `scripts/create-android-fcm-push-evidence-dir.mjs` now define the Android FCM evidence contract and scaffold for 10/10 physical release-device `AwaitingInput` deliveries, inspected hash-only FCM HTTP v1 payloads, generic notification copy, and tap-through to the target daemon-owned session. The AWS live-test bridge now has a relay-only `fieldwork-relay-fcm@fieldwork-oss.iam.gserviceaccount.com` service-account key installed for FCM HTTP v1 sends, but this gate remains unchecked until real APNs/FCM provider delivery is exercised 10/10 on physical devices and inspected payload evidence passes.
...
- [ ] Terminal renders `yes | head -10000` without dropped output on iOS + Android — local mobile-core stress coverage verifies delivery of a `yes | head -10000`-scale byte stream without dropped bytes or offset drift. `pnpm test:android-emulator-flood` is now a local Android renderer substitute: it pairs the actual Android app to an isolated release daemon, opens a desktop-created terminal, renders a `yes | head -10000`-scale stream in the actual Android terminal view, verifies a nonblank flood screenshot, and uses a separately approved verifier client to confirm replayed terminal bytes contain `ANDROID_EMULATOR_FLOOD` output; latest hosted-relay aggregate run on the API 36.1 emulator reported 8437/14400 nonblack screenshot samples. The first physical Android live-test evidence verifier now requires dedicated `flood.png`/`flood-ui.xml`/`flood-logcat.log`/`flood-crash.log` plus `flood-replay.txt` proving `yes ANDROID_LIVE_FLOOD | head -10000` completed with `flood_lines=10000` and at least 10000 replayed marker lines. `docs/ANDROID_RENDERER_FLOOD.md` and `pnpm check:android-renderer-flood-evidence -- "$FW_ANDROID_FLOOD_DIR"` now define the focused Android release-device evidence contract for signed release artifact, direct `adb` terminal screenshot/UI/log/crash capture, and replay with at least 10000 `ANDROID_LIVE_FLOOD` lines. This still needs physical iOS/Android renderer verification before the release gate can be checked.
- [ ] 30-minute physical Android terminal renderer dogfood passes the Section 7.6 termlib decision gate — local Android release artifact validation, debug emulator renderer smokes, direct adb pair/attach/input evidence, and the `yes | head -10000` emulator flood substitute keep development confidence high. `docs/ANDROID_DOGFOOD.md` and `pnpm check:android-dogfood-evidence -- "$FW_DOGFOOD_DIR"` now define the physical evidence contract for a real phone: attached live `claude` session, `dogfood_duration_ms>=1800000`, Android-originated typing, scroll, resize, paste, no Android fatal/ANR logcat entries, no Android system not-responding overlays, and empty crash buffers. The original termlib decision gate still requires that physical Android device dogfood before Play internal distribution.
...
- [ ] Android app cold start: <1200ms — current local API 36.1 emulator evidence is debug-only, not release-device evidence. A 2026-05-19 direct adb emulator QA refresh installed the default debug APK, launched with `Status: ok` and `TotalTime=5297ms`, captured `/tmp/fieldwork-adb-direct-20260519225027/default.png`, `/tmp/fieldwork-adb-direct-20260519225027/default-ui.xml`, `/tmp/fieldwork-adb-direct-20260519225027/default-logcat.log`, and an empty `/tmp/fieldwork-adb-direct-20260519225027/default-crash.log`, and verified the locked `Unlock` surface. The same direct adb run rebuilt the debug APK with `FIELDWORK_ANDROID_BIOMETRIC_BYPASS=true` plus debug-only `FIELDWORK_ANDROID_PAIRING_CODE`, launched the pair build in `TotalTime=4589ms`, tapped the UI-tree-derived Pair center `540 1860`, paired through explicit desktop approval in `pair_flow_ms=1043`, verified `bash · fieldwork`/`ANDROID_ADB_DIRECT_READY`, attached the terminal, sent `fw_android_direct_ok`, captured `/tmp/fieldwork-adb-direct-pair-20260519225208/before-pair.png`, `/tmp/fieldwork-adb-direct-pair-20260519225208/sessions.png`, `/tmp/fieldwork-adb-direct-pair-20260519225208/terminal-before-input.png`, `/tmp/fieldwork-adb-direct-pair-20260519225208/terminal-after-input.png`, UI XML, logcat, and an empty crash buffer, and confirmed a separately approved verifier client saw `android-direct: fw_android_direct_ok` in replayed terminal bytes. Afterward the default debug APK was rebuilt and reinstalled, `BuildConfig.java` restored `FIELDWORK_BIOMETRIC_BYPASS = false` plus `FIELDWORK_DEBUG_PAIRING_CODE = ""`, the restored default build launched in `TotalTime=5105ms`, `/tmp/fieldwork-adb-direct-restore-20260519225316/restored-locked.png` plus `/tmp/fieldwork-adb-direct-restore-20260519225316/restored-ui.xml` verified the locked `Unlock` surface again, and the restored crash buffer remained empty. A 2026-05-20 follow-up direct adb pass paired through explicit desktop approval, attached the session, sent `android_adb_direct_ping`, verified `android-direct: android_adb_direct_ping` in `/tmp/fieldwork-adb-direct-pair-20260519235638/terminal-after-input.png` and PTY output, then installed a biometric-bypass build with empty `FIELDWORK_DEBUG_PAIRING_CODE`; the paired-data relaunch restored the sessions dashboard in `TotalTime=6225ms`, captured `/tmp/fieldwork-adb-direct-pair-20260519235638/relaunch-restore-fix-sessions.png`, and filtered logcat showed `FieldworkRepository: listSessions returned 1 sessions` with no `Camera`/`CAMERA`, Fieldwork `FATAL`, or ANR entries after the saved-pairing restore placeholder fix. A later 2026-05-20 raw adb pass installed the default debug APK, launched the locked app in `TotalTime=6766ms`, captured `/tmp/fieldwork-adb-direct-20260520001909/default-locked.png` and UI/logcat/crash-buffer files, rebuilt with `FIELDWORK_ANDROID_BIOMETRIC_BYPASS=true` plus debug-only `FIELDWORK_ANDROID_PAIRING_CODE`, paired through explicit desktop approval, granted the runtime notification prompt, verified the dashboard listed `bash · fieldwork` with `ANDROID_ADB_MANUAL_READY`, attached the terminal, sent `android_adb_manual_ok` via `adb shell input text` plus Enter, and captured `/tmp/fieldwork-adb-direct-20260520001909/terminal-after-input.png` showing `android-direct: android_adb_manual_ok`. The app logcat showed `FieldworkRepository: pair completed` and `listSessions returned 1 sessions`, crash buffers were empty, and the final restored default debug build had `FIELDWORK_BIOMETRIC_BYPASS = false`, `FIELDWORK_DEBUG_PAIRING_CODE = ""`, launched in `TotalTime=1371ms`, and showed the locked `Unlock` surface at `/tmp/fieldwork-adb-direct-20260520001909/default-restore-locked.png`. AVDs without enrolled biometrics can use the debug-build-only biometric bypass guarded by `BuildConfig.DEBUG`; release builds hardcode the bypass off. Physical release-device cold-start evidence is still required before checking this gate; the Play Store emulator image still emits background Google-service ANRs, so it is not a substitute for the release-device threshold.
  ...
  A 2026-05-21 direct adb pair/attach refresh under `/tmp/fieldwork-adb-direct-20260521165654` installed a debug-only pairing-code build, launched it in `TotalTime=1717ms`, paired through explicit desktop approval, listed the desktop-created `android-direct` session, attached it, sent `fw_direct_20260521_ok` through `adb shell input text`, captured dashboard and terminal screenshots/UI dumps/logcat/crash buffers, and saved a desktop replay at `pair-runtime/pty-replay-after-input.txt` containing `android-direct: fw_direct_20260521_ok`. The default debug APK was then rebuilt/reinstalled with `FIELDWORK_BIOMETRIC_BYPASS = false` and `FIELDWORK_DEBUG_PAIRING_CODE = ""`, relaunched with `Status: ok`, `TotalTime=1862ms`, and the locked `Unlock` surface; crash buffers stayed empty.
  A later 2026-05-21 direct adb locked-launch refresh under `/tmp/fieldwork-adb-direct-20260521-locked-refresh` reinstalled the default debug APK, confirmed `FIELDWORK_BIOMETRIC_BYPASS = false` and `FIELDWORK_DEBUG_PAIRING_CODE = ""`, cleared app data/logcat, launched with `Status: ok`, `LaunchState: COLD`, and `TotalTime=976ms`, captured `locked.png`, `locked-ui.xml`, `logcat.log`, and an empty `crash.log`, verified a 1080x2400 screenshot plus `text="Unlock"` in the UI dump, and found no Fieldwork `FATAL EXCEPTION` or ANR log entries. This remains debug-emulator evidence only, not release-device evidence.
  A 2026-05-23 direct adb emulator pass under `/tmp/fieldwork-adb-direct-20260523103948` refreshed the first-live-test shape without wrapper smoke scripts: the default debug APK launched locked in `TotalTime=1922ms`, a temp npm-layout `fw` shim created the auto-named default `claude` session `widget`, `fw refactoringjob` created the named `claude` shortcut session, `fieldwork new --name shell` created a desktop-owned shell, Android paired through the real Pair UI with explicit desktop approval, and the dashboard showed `widget`, `refactoringjob`, and `shell`. Android attached to `shell`, sent `fw_android_live_ok`, and a desktop attach replay at `terminal-replay-clean.txt` contained `android-direct: fw_android_live_ok`; a force-stop/relaunch restored the paired dashboard in `TotalTime=1266ms` with that scrollback. The debug APK was rebuilt/reinstalled afterward with `FIELDWORK_BIOMETRIC_BYPASS = false` and `FIELDWORK_DEBUG_PAIRING_CODE = ""`, relaunched locked in `TotalTime=1321ms`, and all captured crash buffers plus Fieldwork crash/ANR scans were empty. This remains debug-emulator evidence only, not release-device evidence.
  A 2026-05-25 direct adb emulator refresh under `/tmp/fieldwork-adb-direct-20260525105201` and `/tmp/fieldwork-adb-direct-pair-20260525105508` installed the default debug APK, launched the locked app with `Status: ok` and `TotalTime=3117ms`, captured the locked `Unlock` screenshot/UI/logcat plus an empty crash buffer, then installed a debug-only biometric-bypass/pair-payload build and paired through the real Pair UI plus explicit desktop approval in `pair_flow_ms=549`. The dashboard showed the desktop-created `directbash` interactive shell, Android attached to it, typed `echo fw_android_direct_interactive_ok` through direct `adb shell input text` plus Enter, and a separately approved desktop `fieldwork pair-test --attach directbash` verifier saw `fw_android_direct_interactive_ok` in replayed PTY bytes. The app logcat showed `FieldworkRepository: listSessions returned 2 sessions`, no Fieldwork fatal/ANR entries, and an empty crash buffer. The direct run also exposed that raw `uiautomator dump` can wedge during terminal capture, so the Android terminal-attach evidence scaffold now captures the screenshot first and wraps UI dumps in `FIELDWORK_ANDROID_UI_DUMP_TIMEOUT_SECONDS`-bounded direct-adb capture. This remains debug-emulator evidence only; the physical signed-release phone gates remain unchecked.
  A 2026-05-30 direct adb emulator refresh under `/tmp/fieldwork-adb-direct-20260530042105` paired the current debug app through the real Pair UI and hosted relay typed-code path after explicit desktop approval, showed desktop-created `adbpair` plus `ANDROID_ADB_DIRECT_READY` on the dashboard, attached the live terminal, sent `android_adb_direct_ok` from Android, and saved `desktop-replay.txt` proving the same daemon-owned PTY replay contained `android-direct: android_adb_direct_ok`. The run captured locked, pair, dashboard, terminal, logcat, and crash-buffer evidence, left crash buffers empty, found no Fieldwork fatal/ANR log entries, and rebuilt/reinstalled the default debug APK afterward with `FIELDWORK_BIOMETRIC_BYPASS = false`, `FIELDWORK_DEBUG_PAIRING_CODE = ""`, and `FIELDWORK_RELAY_CONTROL_URL = ""`. This remains debug-emulator evidence only; physical signed-release phone gates remain unchecked.
  A later 2026-05-30 direct adb locked-launch refresh under `/tmp/fieldwork-adb-locked-20260530071152` reinstalled the current default debug APK, cleared app data, confirmed `BuildConfig.java` still has `FIELDWORK_BIOMETRIC_BYPASS = false`, `FIELDWORK_DEBUG_PAIRING_CODE = ""`, and `FIELDWORK_RELAY_CONTROL_URL = ""`, launched `app.fieldwork.android/.MainActivity` with `Status: ok` and `LaunchState: COLD`, captured a 1080x2400 screenshot plus UI XML containing only the locked `Unlock` surface, and found no `FATAL EXCEPTION`, ANR, `am_crash`, or `AndroidRuntime` entries with an empty crash buffer. This is a privacy/locked-surface emulator sanity check after the full local release gate, not release-device cold-start evidence.
  Android release cold-start evidence contract: `docs/ANDROID_COLD_START.md` and `pnpm check:android-cold-start-evidence -- "$FW_ANDROID_COLD_DIR"` require a signed release App Bundle, release `BuildConfig` proof, physical `adb devices -l` evidence, installed `app.fieldwork.android` package proof with `versionName=1.0` and `versionCode=1`, five cold launches of `app.fieldwork.android/.MainActivity` with `TotalTime<=1200ms`, locked-surface screenshot/UI evidence, no Android fatal/ANR logcat entries, no Android system not-responding overlays, and empty crash buffers before the Android cold-start gate can be checked.
  A 2026-05-30 local release APK install smoke converted the current release AAB
  into an ephemeral non-debug signed universal APK, installed it on the API 36.1
  emulator, proved the installed package was non-debuggable with `run-as:
  package not debuggable: app.fieldwork.android`, and launched the release
  package with retained passing launch attempt `TotalTime=1169ms` with the
  locked `Unlock` surface, `launch-attempts.txt` for earlier cold-start
  variance, no Fieldwork fatal/ANR logcat entries, and an empty crash buffer
  under `/tmp/fieldwork-android-release-install-20260530045350`.
  `scripts/verify-android-release-install-evidence.mjs` now verifies this
  captured APKS/static metadata plus direct-adb install/locked-launch evidence,
  and its `--strict-release-device` mode rejects emulator evidence plus the
  local `Fieldwork Release Smoke` certificate for the top-level Android release
  evidence pack. `scripts/test-android-release-install-evidence.mjs` keeps both
  local-smoke and strict release-device verifier paths in the local release
  gate. `scripts/create-android-release-install-evidence-dir.mjs`
  now makes this bundletool, ephemeral non-debug signing, direct-adb install,
  and locked-launch capture repeatable with separate `apks/` and `install/`
  evidence directories; `scripts/test-android-release-install-scaffold.mjs`
  verifies the scaffold mirrors the verifier without fabricating evidence. This
  improves local release-build confidence but remains emulator evidence; the
  physical release-device cold-start gate still requires five launches on a real
  phone. In short, this local release-install smoke requires five launches on a real phone before the physical cold-start gate can be checked.
  Android cold-start evidence scaffold note (2026-05-24): `scripts/create-android-cold-start-evidence-dir.mjs` now creates the Android release-device cold-start evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, install transcript, installed package identity/version, five cold launch transcripts, locked-surface screenshot/UI dump, logcat, and crash buffer, then runs `scripts/verify-android-cold-start-evidence.mjs`. `scripts/test-android-cold-start-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator/debuggable/slow-launch evidence, and does not replace the physical signed-release phone gate.
  Android physical-release evidence hardening note: the first live-test verifier and every focused Android release-device verifier now require `package-info.txt` from `adb shell pm path app.fieldwork.android` plus `adb shell dumpsys package app.fieldwork.android`, proving the tested phone has `app.fieldwork.android` installed with `versionName=1.0` and `versionCode=1`. Focused release-device verifiers also reject installed-package evidence containing `DEBUGGABLE` or `debuggable=true`; the first-round live-testing verifier remains debug-build-friendly but still requires normal no-bypass `BuildConfig` proof. Signed AAB output, release `BuildConfig`, or an install transcript alone is not enough evidence for any Android physical-device gate.
- [ ] Reconnect after network change: <2s — local handoff smoke now detaches the simulated iroh phone while a PTY emits missed output, reconnects with `last_seen_seq`, and verifies replay arrives through `Attached.initial_bytes` within 2 seconds. Android source now also records the latest mobile `lastSeenSeq`, destroys the broken attachment, and reattaches/restarts the byte subscription after an attached-stream error; focused Android JVM coverage verifies the reattach starts from the latest offset. `pnpm test:android-emulator-reconnect` is the local Android-app substitute: it pairs the actual Android app to an isolated release daemon, opens a desktop-created terminal, sends input before and after an emulator airplane-mode network cut, confirms the desktop PTY receives post-restore input, and uses a separately approved verifier client to confirm output emitted during the network gap remains replayable. `docs/ANDROID_NETWORK_RECONNECT.md` and `pnpm check:android-network-reconnect-evidence -- "$FW_ANDROID_RECONNECT_DIR"` now define the physical release-device evidence contract for signed release artifact, direct `adb` network cut/restore, replay of offline PTY output, `reconnect_ms<=2000`, post-reconnect Android-originated input, no Android fatal/ANR logcat entries, no Android system not-responding overlays, and empty crash buffers. Physical-device timing remains required before checking the gate.
  Android network reconnect evidence scaffold note (2026-05-24): `scripts/create-android-network-reconnect-evidence-dir.mjs` now creates the Android release-device network reconnect evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, installed package identity/version, before-cut UI, airplane-mode cut proof, network restore ping proof, after-restore UI, logcat, and crash buffer. It still requires real `fw attach` replay transcripts and observed `reconnect_ms=<elapsed-ms>` before verification; `scripts/test-android-network-reconnect-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator/debuggable evidence, and does not fabricate PTY replay or reconnect timing.
- [ ] Pair flow end-to-end: <15s including QR scan and desktop confirmation prompt — the hidden local handoff simulator still covers QR payload → iroh pair-test → desktop approval within 15 seconds. `pnpm test:android-emulator-pair` now also measures the actual Android debug-app pairing path from tapping the debug-injected Pair button through explicit desktop approval completion and fails above the local 15-second emulator bound; latest focused run passed on `emulator-5554` with `pair_flow_ms=2206`. A 2026-05-20 direct adb source-build `fw` shim pass, without wrapper smoke scripts, created desktop sessions through bare `fw`, `fw refactoringjob`, and `fw new --name shell`, then paired the Android debug app through explicit desktop approval in `pair_flow_ms=423`; evidence includes `/tmp/fieldwork-fw-direct-pair-20260520152507/dashboard.png`, `/tmp/fieldwork-fw-direct-pair-20260520152507/after-pair.xml`, `/tmp/fieldwork-fw-direct-pair-20260520152507/dashboard-logcat.log`, and an empty `/tmp/fieldwork-fw-direct-pair-20260520152507/dashboard-crash.log`. The 2026-05-23 direct adb pass under `/tmp/fieldwork-adb-direct-20260523103948` repeated the actual Android Pair UI path from a fresh daemon, required explicit desktop approval, and produced dashboard/logcat/crash evidence for `widget`, `refactoringjob`, and `shell` with no Fieldwork fatal/ANR entries. The 2026-05-30 direct adb pass under `/tmp/fieldwork-adb-direct-20260530042105` repeated the hosted relay typed-code Pair UI path on the current debug app and proved Android-originated terminal input through `desktop-replay.txt`. `docs/ANDROID_PAIR_FLOW.md` and `pnpm check:android-pair-flow-evidence -- "$FW_ANDROID_PAIR_DIR"` now define the physical release-device evidence contract for a signed release app installed as `app.fieldwork.android` with `versionName=1.0` and `versionCode=1`, real QR scanner, explicit desktop approval, `pair_flow_ms<=15000`, post-pair dashboard showing desktop-created sessions, no Android fatal/ANR logcat entries, no Android system not-responding overlays, and empty crash buffers. This substitutes for the app-side timing path until physical camera QR scan timing is available.
  Android pair-flow evidence scaffold note (2026-05-24): `scripts/create-android-pair-flow-evidence-dir.mjs` now creates the Android release-device pair-flow evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, and installed package identity/version before the QR scan, clears Android logs, then after the real `fw pair` transcript exists reruns with `FIELDWORK_ANDROID_PAIR_CAPTURE_DASHBOARD=true` to capture dashboard screenshot/UI, `fw ls`, `fw devices`, logcat, and crash buffer before running `scripts/verify-android-pair-flow-evidence.mjs`. `scripts/test-android-pair-flow-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator/debuggable evidence, and does not fabricate `pairing.txt` or bypass explicit desktop approval.
...
- [ ] Pair a fresh phone to a fresh daemon, see sessions list — local substitutes cover the app-side path: `pnpm test:android-emulator-pair` pairs the Android debug app to an isolated release daemon through the hosted-relay typed-code path, and the latest direct adb pair/attach pass paired the actual Android app through explicit desktop approval, listed `adbpair`, attached the terminal, sent `android_adb_direct_ok`, and verified the PTY replay through screenshots, logcat, and `desktop-replay.txt`. A direct adb source-build `fw` shim pass also verified the first-live-test dashboard path: `fw ls` listed auto-named `kazoo`, `refactoringjob`, and `shell`; Android dashboard XML and screenshot at `/tmp/fieldwork-fw-direct-pair-20260520152507/after-pair.xml` and `/tmp/fieldwork-fw-direct-pair-20260520152507/dashboard.png` showed the same three sessions with no `No sessions` state; app logcat showed `FieldworkRepository: pair completed` and `FieldworkRepository: listSessions returned 3 sessions`; and `/tmp/fieldwork-fw-direct-pair-20260520152507/dashboard-crash.log` was empty. A 2026-05-23 direct adb refresh under `/tmp/fieldwork-adb-direct-20260523103948` then repeated the same first-live-test shape with the short `fw` alias, showing auto-named `widget`, `refactoringjob`, and `shell` on Android, proving Android-originated shell input in `terminal-replay-clean.txt`, restoring paired dashboard after relaunch, and restoring the default debug APK to an empty debug pairing code. The 2026-05-30 direct adb refresh under `/tmp/fieldwork-adb-direct-20260530042105` repeated the current hosted relay typed-code pair/attach/input path and restored the debug APK to empty bypass/pairing/relay debug values afterward. `docs/ANDROID_PAIR_FLOW.md` now also requires release-device dashboard evidence after real QR pairing, including `refactoringjob`, a desktop-created `shell`/`bash` session, `fw devices`, and repository pair/list logcat. Physical-phone QR scan evidence remains required before checking this gate.
- [ ] **Create session from desktop CLI** (`fw new --dir ~/projects claude`), watch it appear in the phone's session list within 2 seconds (over iroh subscription), tap in, type, see output. Mobile cannot create sessions per Section 6.4 — this smoke test exercises the desktop-creates / phone-attaches flow. The hidden local handoff simulator covers the 2-second subscription path; `pnpm test:android-emulator-session-subscription` is the actual Android-app local substitute that pairs with no pre-existing sessions, creates `fw_subscribe_session` from the desktop CLI, verifies the subscribed dashboard receives it within the local 8-second emulator bound, opens it, sends `subscription_attach_ok`, and confirms the PTY receives that Android-originated input. The smoke now recovers Fieldwork foreground before UI dumps and falls back to file-backed `uiautomator` dumps when direct streaming hangs; latest focused run passed on `emulator-5554` with `visible_ms=2904`. `docs/ANDROID_SESSION_SUBSCRIPTION.md` and `pnpm check:android-session-subscription-evidence -- "$FW_ANDROID_SUBSCRIPTION_DIR"` now define the physical release-device evidence contract: after real QR pairing, `fw_live_sub` must be created from the desktop CLI, absent from the pre-create Android dashboard, visible on Android with `visible_ms=<elapsed-ms>` at or below 2000, and replayable through `subscription-replay.txt` with `subscription_attach_ok` from Android-originated input. `docs/ANDROID_TERMINAL_ATTACH.md` and `pnpm check:android-terminal-attach-evidence -- "$FW_ANDROID_TERMINAL_DIR"` now define the broader Android terminal attach evidence contract for signed release devices: Android attaches to desktop-created shell, Claude, and `vim`/`htop` sessions, shell replay contains `android_live_ok`, Claude replay contains `claude_live_ok` without the shell marker, TUI UI evidence shows `Attached` plus real `vim`/`htop` content, and mobile UI does not expose create/kill/command controls. `docs/ANDROID_RESIZE_DETACH.md` and `pnpm check:android-resize-detach-evidence -- "$FW_ANDROID_RESIZE_DIR"` now define Android resize/detach evidence: resize replay contains a plausible `resize_size` plus `after_resize_ok`, and detach replay contains `after_detach_reattach_ok` after Android reattaches to the same PTY. Physical-phone QR scan, 2-second release-device timing, physical release-device terminal attach, and physical release-device resize/detach evidence remain required before checking this gate.
  Android session-subscription evidence scaffold note (2026-05-24): `scripts/create-android-session-subscription-evidence-dir.mjs` now creates the Android release-device session-subscription evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, and installed package identity/version before the test, captures `dashboard-before-ui.xml` in `FIELDWORK_ANDROID_SUBSCRIPTION_CAPTURE_BEFORE=true` mode, and after the real desktop-create, visibility timing, and replay transcripts exist, `FIELDWORK_ANDROID_SUBSCRIPTION_CAPTURE_AFTER=true` captures screenshot/UI/log/session-list evidence before running `scripts/verify-android-session-subscription-evidence.mjs`. `scripts/test-android-session-subscription-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator evidence, and does not fabricate the desktop-created session, visibility timing, or Android-originated input replay.
  Android terminal attach evidence scaffold note (2026-05-24): `scripts/create-android-terminal-attach-evidence-dir.mjs` now creates the Android release-device terminal attach evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, and installed package identity/version before the test, captures staged shell/Claude/TUI screenshot/UI/log/crash evidence with `FIELDWORK_ANDROID_TERMINAL_CAPTURE_SHELL=true`, `FIELDWORK_ANDROID_TERMINAL_CAPTURE_CLAUDE=true`, and `FIELDWORK_ANDROID_TERMINAL_CAPTURE_TUI=true`, and runs `scripts/verify-android-terminal-attach-evidence.mjs` in `FIELDWORK_ANDROID_TERMINAL_VERIFY=true` mode after the real desktop replay transcripts exist. `scripts/test-android-terminal-attach-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator evidence, and does not fabricate desktop sessions or PTY replay transcripts.
  Android resize/detach evidence scaffold note (2026-05-24): `scripts/create-android-resize-detach-evidence-dir.mjs` now creates the Android release-device resize/detach evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, and installed package identity/version before the test, captures staged resize/detach screenshot/UI/log/crash evidence with `FIELDWORK_ANDROID_RESIZE_CAPTURE_RESIZE=true` and `FIELDWORK_ANDROID_RESIZE_CAPTURE_DETACH=true`, and runs `scripts/verify-android-resize-detach-evidence.mjs` in `FIELDWORK_ANDROID_RESIZE_VERIFY=true` mode after the real desktop replay transcripts exist. `scripts/test-android-resize-detach-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator evidence, and does not fabricate desktop sessions or PTY replay transcripts.
  Android biometric evidence scaffold note (2026-05-24): `scripts/create-android-biometric-evidence-dir.mjs` now creates the Android release-device biometric evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, and installed package identity/version before the test, captures staged locked-launch, BiometricPrompt, and stale-prompt screenshot/UI/log/crash evidence with `FIELDWORK_ANDROID_BIOMETRIC_CAPTURE_LOCKED=true`, `FIELDWORK_ANDROID_BIOMETRIC_CAPTURE_PROMPT=true`, and `FIELDWORK_ANDROID_BIOMETRIC_CAPTURE_STALE=true`, and runs `scripts/verify-android-biometric-evidence.mjs` in `FIELDWORK_ANDROID_BIOMETRIC_VERIFY=true` mode after the real stale-input transcript exists. `scripts/test-android-biometric-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator evidence, and does not fabricate pairing, sessions, or stale-input proof.
  Android dogfood evidence scaffold note (2026-05-24): `scripts/create-android-dogfood-evidence-dir.mjs` now creates the Android release-device renderer dogfood evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures release `BuildConfig`, physical device listing, and installed package identity/version before the test, captures staged Claude, scroll, resize, paste, and final log/crash evidence with `FIELDWORK_ANDROID_DOGFOOD_CAPTURE_CLAUDE=true`, `FIELDWORK_ANDROID_DOGFOOD_CAPTURE_SCROLL=true`, `FIELDWORK_ANDROID_DOGFOOD_CAPTURE_RESIZE=true`, `FIELDWORK_ANDROID_DOGFOOD_CAPTURE_PASTE=true`, and `FIELDWORK_ANDROID_DOGFOOD_CAPTURE_FINAL=true`, and runs `scripts/verify-android-dogfood-evidence.mjs` in `FIELDWORK_ANDROID_DOGFOOD_VERIFY=true` mode after the real 30-minute duration and desktop replay transcripts exist. `scripts/test-android-dogfood-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator evidence, and does not fabricate the dogfood duration, operator scroll review, or PTY replay transcripts.
  Android renderer flood evidence scaffold note (2026-05-24): `scripts/create-android-renderer-flood-evidence-dir.mjs` now creates the Android release-device renderer flood evidence directory scaffold from the verifier's required-file list. Its direct-adb `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, and installed package identity/version before the test, captures flood screenshot/UI/log/crash evidence with `FIELDWORK_ANDROID_FLOOD_CAPTURE=true`, and runs `scripts/verify-android-renderer-flood-evidence.mjs` in `FIELDWORK_ANDROID_FLOOD_VERIFY=true` mode after the real desktop replay transcript exists. `scripts/test-android-renderer-flood-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator evidence, and does not fabricate desktop sessions, the flood command, or PTY replay transcripts.
...
- [ ] Kill daemon, restart, sessions list shows last-known sessions (scrollback restored, processes died — documented) — the hidden local handoff simulator verifies desktop restore. `pnpm test:android-emulator-restart-restore` is the actual Android-app local substitute: it pairs the debug app with an isolated release daemon, creates an intentionally completed `fw_restart_session` so the daemon persists `ANDROID_RESTART_SCROLLBACK` through the session-exit path, restarts the daemon with the same persisted state and node identity, relaunches the app from saved pairing, verifies the restored dashboard still shows `fw_restart_session`, opens the restored terminal, and uses a separately approved verifier to confirm `ANDROID_RESTART_SCROLLBACK` is replayed from restored scrollback. The repeatable smoke passed on `emulator-5554` on 2026-05-19 after the ViewModel main-thread fix. Direct adb restart-restore evidence on 2026-05-19 captured the paired dashboard and logcat before/after a daemon restart, exposed an Android `ANR in app.fieldwork.android` when refresh performed mobile-core session listing on the main thread, then passed after `FieldworkViewModel` moved repository calls to `Dispatchers.IO`: screenshots showed `fw_restart_session` on the restored dashboard before and after refresh, logcat showed `FieldworkRepository: listSessions returned 1 sessions`, and no Fieldwork `FATAL EXCEPTION` or ANR remained. `docs/ANDROID_RESTART_RESTORE.md` and `pnpm check:android-restart-restore-evidence -- "$FW_ANDROID_RESTART_DIR"` now define the physical release-device evidence contract for signed release builds: after `fw daemon restart`, the Android app must relaunch from saved pairing, show or attach `fw_restart_session`, and `restart-replay.txt` must contain restored `ANDROID_RESTART_SCROLLBACK`; `daemon-restart.txt` must also document `processes_died_documented` because live PTY processes are not expected to survive daemon restart.
  Android restart-restore evidence scaffold note (2026-05-24): `scripts/create-android-restart-restore-evidence-dir.mjs` now creates the Android release-device daemon restart restore evidence directory scaffold from the verifier's required-file list. Its direct-adb/local-`fw` `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, installed package identity/version, `fw ls` seed state, `fw devices`, `fw daemon restart` timing plus `processes_died_documented`, restored Android UI, logcat, and crash buffer. It still requires a real `fw attach fw_restart_session` replay transcript proving restored `ANDROID_RESTART_SCROLLBACK`; `scripts/test-android-restart-restore-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator/debuggable evidence, and does not fabricate the restored PTY replay.
- [ ] Run 3 sessions in parallel, switch between them on phone, no state leakage — the hidden local handoff smoke verifies switched simulated-phone sessions do not receive each other's output markers, and `pnpm test:android-emulator-multisession` is the actual Android-app substitute: it opens `fwm_a`, `fwm_b`, and `fwm_c`, switches among all three in the app, sends Android-originated input to each, and verifies `multi_a_ok`, `multi_b_ok`, and `multi_c_ok` land only in their selected PTYs. Latest focused run on 2026-05-29 passed on `emulator-5554` after hosted-relay typed-code hardening. `docs/ANDROID_MULTISESSION.md` and `pnpm check:android-multisession-evidence -- "$FW_ANDROID_MULTISESSION_DIR"` now define the physical release-device evidence contract for signed release builds: Android must switch among desktop-created `fwm_a`/`fwm_b`/`fwm_c`, each replay file must contain only its selected marker, and the verifier rejects cross-session marker leakage or mobile create/kill/command controls. Physical-device switching remains required before checking this gate.
  Android multisession evidence scaffold note (2026-05-24): `scripts/create-android-multisession-evidence-dir.mjs` now creates the Android release-device multisession evidence directory scaffold from the verifier's required-file list. Its direct-adb/local-`fw` `preflight.sh` captures signed artifact proof, release `BuildConfig`, physical device listing, installed package identity/version, `fw ls` proof of `fwm_a`/`fwm_b`/`fwm_c`, switched Android UI, logcat, and crash buffer. It still requires real per-session `fw attach` replay transcripts proving selected markers do not leak across PTYs; `scripts/test-android-multisession-scaffold.mjs` and `pnpm check:local-release` verify that the scaffold mirrors the verifier, rejects emulator/debuggable evidence, and does not fabricate replay files.

**Local substitute note (2026-05-23)**: `scripts/smoke-local-handoff.sh`
...
--with-runtime` run paired in 2 seconds, created default `claude`, `bash`,
...
replayed missed output after a simulated iroh reconnect within 2 seconds (17ms
...
`scripts/create-live-testing-pack.mjs` and
`pnpm scaffold:live-testing-pack -- --print-dir` now create the first-round
Android live-test workspace in one step: a source-checkout `fw`/`fieldwork`/
`fieldworkd` shim under `bin/`, the live-test evidence scaffold under
`evidence/`, `setup.sh` exporting `FW_LIVE_PACK`/`FW_LIVE_BIN`/`FW_LIVE_DIR`,
and a top-level `preflight.sh` that runs local readiness, runs `fw doctor` to
prove the desktop CLI can start and handshake with `fieldworkd`, then delegates
to the direct-`adb` evidence preflight. The generated preflight now requires the
normal first-round debug `BuildConfig`: no biometric bypass, no embedded pairing
code, and no `FIELDWORK_RELAY_CONTROL_URL` override. The pack does not fabricate
evidence or replace npm/provenance/platform/release-signing/physical-device
gates, and `scripts/test-live-testing-pack.mjs` plus `pnpm check:local-release`
keep it wired.
...
through the hosted-relay typed-code path, computes a real desktop session's
...
through the hosted-relay typed-code path, opens three desktop-created
...
Latest focused run on 2026-05-29 passed on `emulator-5554`.
...
input. The smoke now recovers Fieldwork foreground before UI dumps and falls
back to file-backed `uiautomator` dumps when direct streaming hangs. Latest
focused run passed on `emulator-5554` with `visible_ms=2904`.
...
`FIELDWORK_DEBUG_PAIRING_CODE = ""` with the locked `Unlock` surface at
...
- `cli::commands::pair` — generates a 5-char code via `OsRng`, builds a `PairingTicket { code, node_id, relay_url, addrs, expires_at }`, prints the encoded `fw1…` ticket as a QR via the `qrcode` crate and the bare code for manual entry, and best-effort publishes `code → ticket_blob` to the relay when `FIELDWORK_RELAY_CONTROL_URL` is set.
- A hidden Rust test client, `fieldwork pair-test`, takes the encoded `fw1…` ticket as `--payload` (or stdin), or a `--code` with `--relay-control-url` to resolve via the relay first, then round-trips `ListSessions` and optional `AttachSession` over iroh.
- Mutual auth: `PairWithCode` over iroh + Ed25519 pubkey exchange; after desktop approval, the long-lived device identity is stored in encrypted `devices.redb` under a hashed device row key.
...
**Android FCM push evidence scaffold note (2026-05-23)**:
`scripts/create-android-fcm-push-evidence-dir.mjs` now creates the Android FCM
provider-push evidence directory scaffold from the verifier's required-file list
and writes only helper files plus a direct-adb `preflight.sh`. The preflight can
capture signed release artifact proof, release BuildConfig proof, relay version
output, one physical Android device listing, and installed package
identity/version, while rejecting emulators, debug builds, biometric bypass,
debug pairing codes, missing package version proof, and debuggable package
markers. It does not create provider payload JSON, delivery-count evidence,
notification screenshots, tap UI evidence, replay transcripts, or logs.
`docs/ANDROID_FCM_PUSH.md` uses the scaffold and
`scripts/test-android-fcm-push-scaffold.mjs` plus `pnpm check:local-release`
verify that the scaffold mirrors `scripts/verify-android-fcm-push-evidence.mjs`
and still fails the Android FCM verifier until real provider-payload, delivery,
notification, tap-through, replay, and log evidence is captured.
...
- Write the `release-rust.yml` workflow: cross-compile **4 host targets** (darwin-arm64, darwin-x64, linux-x64, linux-arm64) via cargo-zigbuild for Linux + native runners for Darwin, ad-hoc sign Darwin CLI/daemon binaries for the npm path, optionally Developer ID sign/notarize Darwin audit artifacts when Apple credentials exist, and upload artifacts.
...
**Release-artifact evidence contract (2026-05-25)**: `scripts/create-release-artifacts-evidence-dir.mjs` now scaffolds the release-rust/GitHub Release artifact evidence directory without creating release artifacts or running GitHub workflows. Its `preflight.sh` records downloaded artifact SHA-256 digests and runs `pnpm check:release-artifacts` with `FIELDWORK_VERIFY_COSIGN_SIGNATURE=1`, `FIELDWORK_EXPECTED_RELEASE_TAG=v1.0.0`, and the expected `release-rust.yml@refs/tags/v.*` cosign identity regex. `scripts/verify-release-artifacts-evidence.mjs` requires sanitized workflow success evidence, GitHub Release asset metadata for all twelve archive/checksum/bundle files, downloaded artifact digests, and cosign-backed verifier output. `pnpm test:release-artifacts-evidence`, `pnpm test:release-artifacts-scaffold`, `pnpm check:release-artifacts-evidence`, and `pnpm scaffold:release-artifacts-evidence` are wired into package scripts, local release aggregation, and CI static checks.

**Release workflow secret hygiene refresh (2026-05-19)**: `release-rust.yml`
optionally decodes `cert.p12` and `app-store-key.json` signing/notarization assets when Apple credentials are present
...
`scripts/verify-release-workflows.mjs` pins those behaviors and now extracts
literal GitHub Actions `run: |` blocks for `bash -n`, with a self-test that
proves malformed shell such as a missing `fi` is rejected. Future release
workflow edits cannot silently leave decoded Apple, Firebase, or signing assets
or relay SSH keys in the repository workspace, world-readable, persistent in the
runner workspace, or hidden behind invalid shell syntax.

**Distribution note update (2026-05-18)**: the npm name model is now unscoped. `fieldwork` is the v1 meta package, and the four platform children are `fieldwork-darwin-arm64`, `fieldwork-darwin-x64`, `fieldwork-linux-arm64`, and `fieldwork-linux-x64`. The five publishable npm manifests and Rust workspace packages are set to `1.0.0`; rebuilt host and cross-target release binaries contain `fieldwork 1.0.0`. The unscoped `fieldwork` meta package is operator-owned, so no further npm name-availability checks are needed for it. `scripts/verify-npm-registry-state.mjs` is reserved for release-state checks and now fails closed without explicit expectation flags: `--expect-platform-published` after operator-controlled platform child publishes and `--expect-latest-version=1.0.0 --expect-provenance` for post-release dist-tag and npm SLSA provenance verification; `scripts/test-npm-registry-state.mjs` covers those modes plus bare-invocation failure with a deterministic local registry fixture. Local desktop release binaries have been staged into all four platform package directories, and `scripts/publish-npm-packages.mjs` rejects non-native platform children before publish in both `--check-ready` and actual publish paths. Those staged `packages/cli-*/bin/fieldwork` and `packages/cli-*/bin/fieldworkd` files are generated release artifacts; `.gitignore` keeps them out of source control while `scripts/verify-npm-packages.mjs --require-binaries` still verifies them when present. The full staged `pnpm check:secret-boundaries` scan covered 32 non-relay artifacts across package bins, desktop release targets, and Android mobile-core release target outputs for relay-only credentials and npm auth-token patterns. The current retained-artifact set includes staged desktop/npm binaries plus debug/release CLI and mobile-core outputs, and the latest `pnpm check:secret-boundaries` scan covered 42 retained non-relay artifacts while still rejecting committed npm token strings or `.npmrc` files.

**npm release evidence contract (2026-05-25)**: `scripts/create-npm-release-evidence-dir.mjs` now scaffolds the operator-owned npm release evidence directory without publishing packages or querying package-name availability. Its `preflight.sh` writes `publish-plan.json` from `scripts/publish-npm-packages.mjs --publish-plan-json` and `publish-readiness.txt` from `--check-ready`; after the real `release-npm.yml` publish, `scripts/verify-npm-release-evidence.mjs` requires sanitized workflow success evidence, children-first publish logs, public registry-state/provenance output, and package metadata proving exactly the five unscoped `1.0.0` packages have npm SLSA provenance. It also rejects legacy scoped `@fieldwork/*` package names and extra unscoped Fieldwork package names in captured logs and metadata so the release evidence cannot drift back to the old scoped distribution model or include out-of-scope v1 packages. `pnpm test:npm-release-evidence`, `pnpm test:npm-release-scaffold`, `pnpm check:npm-release-evidence`, and `pnpm scaffold:npm-release-evidence` are wired into package scripts, local release aggregation, and CI static checks.

**Local npm artifact staging note (2026-05-22)**:
`scripts/build-local-npm-artifacts.sh` now makes the staged npm package
readiness path repeatable on this Mac. It builds host release
`fieldwork`/`fieldworkd`/`fieldwork-relay`, builds Darwin arm64/x64 package
binaries with Cargo, builds Linux x64/arm64 package binaries with
`cargo zigbuild`, stages the generated `packages/cli-*/bin/fieldwork` and
`fieldworkd` files, ad-hoc signs and quarantine-cleans the two Darwin package
directories with `codesign --force --sign -` plus targeted `xattr -d`, verifies
those staged Darwin bins with `scripts/verify-macos-signing.mjs`, copies root
`LICENSE`/`NOTICE` into all five npm package directories, and then runs
`scripts/verify-npm-packages.mjs --require-binaries` plus
`scripts/publish-npm-packages.mjs --check-ready`. It also writes local
platform-package tarballs to `FIELDWORK_LOCAL_NPM_ARCHIVE_DIR` or
`target/local-npm-artifacts`, verifies the Darwin tarballs with the same
macOS npm trust verifier, and writes `.sha256.local` files for local candidate
inventory only. The script is a local release-candidate helper only;
`release-npm.yml` still publishes from verified GitHub Release archives and
...
- Keep daemon/mobile diagnostics local-only unless the v1 contract is updated.
...
**Supply-chain audit refresh (2026-05-29)**: `cargo deny check` passed with advisories, bans, licenses, and sources ok after explicitly documenting the current RustSec warning set. `cargo audit` scanned 748 dependencies and exited successfully with five allowed warnings: `adler` `RUSTSEC-2025-0056`, `atomic-polyfill` `RUSTSEC-2023-0089`, `bincode` `RUSTSEC-2025-0141`, `paste` `RUSTSEC-2024-0436`, and `lru` `RUSTSEC-2026-0002`. `bincode` remains intentionally retained for v1 local IPC under length-prefixed frame limits and round-trip tests. `atomic-polyfill` is transitive through `postcard -> heapless` for compact `fw1` pairing-ticket encoding and iroh's relay path; `cargo update -p postcard --dry-run` found no compatible lockfile move, and `scripts/verify-rust-workspace.mjs` rejects direct `atomic-polyfill` dependencies plus `atomic_polyfill::` source paths. The remaining warnings are transitive through terminal, network, or image dependencies.
...
**CLI doctor smoke refresh (2026-05-24)**: `fieldwork doctor` / `fw doctor`
is now covered by `scripts/smoke-cli-doctor.sh`. The smoke builds the debug
CLI/daemon in an isolated temp `HOME`/XDG runtime/config/state/cache
environment, proves `fieldwork doctor --no-start` fails before a daemon socket
exists, starts `fieldworkd`, creates a desktop `bash` session, then runs
`fw doctor --no-start` through a temp `fw` alias. It verifies the colocated
daemon binary check, socket parent/file hardening, socket/protocol handshake,
`CONTRACT_VERSION = 2` display, push-disabled state, session count, telemetry
status, scrollback-encryption status, summary, and `fw doctor --help` alias usage. `pnpm test:cli-doctor`
exposes the smoke directly, and `pnpm check:local-release:full`
runs it before the no-args and local handoff smokes.

**Structured asset gate note (2026-05-21, portable fallback 2026-05-30)**: `scripts/verify-structured-assets.mjs` is now part of `pnpm check:local-release`, making the PLAN's structured-file syntax promise explicit in the aggregate gate. It parses tracked repo JSON assets with `JSON.parse`, parses tracked TOML assets with Python's standard `tomllib`, lints the iOS project metadata/Info.plist/entitlements with `plutil -lint` when available, uses a portable XML-plist parse plus Xcode project structural fallback on non-macOS hosts, and validates Android XML resources plus docs SVG assets with `xmllint --noout` when available or Python's standard XML parser on hosts without `xmllint`.

**Telemetry scope update note (2026-05-26, tightened 2026-05-29)**: daemon and mobile crash-reporting SDKs were removed from the v1 contract. `fieldwork settings telemetry on` now persists a local diagnostics preference only, mobile diagnostics consent remains local-only, and relay Honeycomb remains the sole hosted observability path in v1. `scripts/verify-telemetry-privacy.mjs` now also rejects removed crash SDK declarations in Android build files and scans existing Android APK/AAB outputs for removed crash SDK class or DSN markers.

**Relay Honeycomb evidence scaffold note (2026-05-23)**:
`scripts/create-relay-honeycomb-evidence-dir.mjs` now creates the hosted relay
Honeycomb receipt evidence directory scaffold from the verifier's required-file
list and writes only helper files plus a non-secret `preflight.sh`. The
preflight can capture relay version output, `/v1/version` request proof,
redacted relay OTLP config, and systemd `honeycomb-api-key` credential wiring
from a production or release-candidate relay host, but it does not export hosted
Honeycomb query rows, create redacted relay logs, or fabricate passing evidence.
`docs/RELAY_HONEYCOMB.md` uses the scaffold and
`scripts/test-relay-honeycomb-scaffold.mjs` plus `pnpm check:local-release`
verify that the scaffold mirrors `scripts/verify-relay-honeycomb-evidence.mjs`
and still fails the hosted receipt verifier until real hosted query rows and
relay logs are captured.
...
**Synthetic snapshot test note (2026-05-18, stabilized 2026-05-30)**: the Section 6.3/13.7 stale-attach gate is now covered by `session::snapshot_tests::stale_attach_snapshot_rehydrates_real_vim_session` plus deterministic `terminal_model::tests::snapshot_rehydrates_visible_cells_attrs_and_cursor`. The real-PTY test starts `vim /etc/hosts`, waits for vim's alt-screen content in the daemon's `wezterm-term` model, forces the stale attach path with an out-of-window `last_seen_seq`, feeds `Attached.initial_bytes` into a fresh in-process `wezterm-term` client model, and asserts the client reaches the same alt-screen mode, cursor position, and rendered visible text as the daemon model. The deterministic terminal-model test keeps exact cell/attribute/cursor snapshot correctness covered without depending on host-specific live Vim blank-cell materialization. The Rust CI matrix installs `vim` on Ubuntu before `cargo nextest run --workspace`, so both gates are part of the normal workspace test suite.

**Performance note (2026-05-18)**: `scripts/measure-desktop-performance.mjs` measures release-build desktop cold starts without touching the user's real Fieldwork state. It runs one explicit warm-up sample to avoid build-machine first-exec page-cache/code-signing noise, then runs measured samples of `target/release/fieldwork version`, starts `target/release/fieldworkd` in isolated temp `HOME`/`XDG_RUNTIME_DIR` directories, and waits until `target/release/fieldwork daemon status` completes a real local IPC handshake with the daemon. Local release build command: `cargo build --release -p fieldwork-cli -p fieldwork-daemon`. The verifier fails on max measured sample, not p95. Latest passing runtime run on this Mac, from `pnpm check:local-release:full` on 2026-05-30, measured CLI median `2.71ms`, p95 `3.08ms`, max `3.47ms` over 25 samples; daemon ready-to-handshake median `39.18ms`, p95 `40.77ms`, max `41.51ms` over 25 samples. `scripts/smoke-local-handoff.sh` now measures the simulated QR payload → iroh pair-test → desktop approval path in whole seconds and fails above 15 seconds; this remains the closest local substitute for physical QR camera timing. `scripts/smoke-android-debug.sh` is the repeatable local Android emulator substitute: latest wiped API 36.1 AVD run installed the debug app, launched `app.fieldwork.android/.MainActivity` with `am start -W` `TotalTime=2467ms`, confirmed the locked `Unlock` surface through `uiautomator`, found no Fieldwork crash-buffer entry, and verified a nonblank 1080x2400 `screencap` with 14391/14400 nonblack samples. A later raw `adb` QA refresh launched the default debug APK in `TotalTime=2297ms`, launched the emulator-only biometric-bypass build in `TotalTime=1460ms`, paired the app to an isolated release daemon in `TotalTime=1297ms`, verified `fw_android_direct_ok` round-tripped through the attached PTY, then rebuilt and relaunched the restored default locked build in `TotalTime=1097ms` with `FIELDWORK_BIOMETRIC_BYPASS = false` and an empty `FIELDWORK_DEBUG_PAIRING_CODE`. `FIELDWORK_ANDROID_BIOMETRIC_BYPASS=true pnpm test:android-debug-smoke` is available for AVDs with no enrolled biometrics and verifies the unlocked pairing/bottom-navigation UI through a debug-build-only bypass guarded by `BuildConfig.DEBUG`; release builds hardcode the bypass off. `pnpm test:android-emulator` now aggregates the direct-adb Android substitutes; the latest hosted-relay aggregate run on 2026-05-29 passed on `emulator-5554` with locked debug launch `TotalTime=6448ms`, pair `pair_flow_ms=1420`, session subscription `visible_ms=5493`, and 8437/14400 flood screenshot nonblack samples while also covering background replay, restart restore, multisession, reconnect, and notification tap routing. The local Android startup path keeps the encrypted pairing store lazy and restores saved pairing on `Dispatchers.IO`, with focused JVM coverage proving ViewModel construction does not block on saved-pairing restore. The Android root uses an explicit Material color scheme plus explicit lock-button colors so the unauthenticated surface does not rely on system dark-mode defaults. The Play Store emulator image still emits background Google-service ANRs, so iOS/Android release cold start, terminal flood, network-change reconnect, physical QR camera scan timing, and physical pair-flow timing remain blocked on physical devices.

**Android aggregate emulator QA note (2026-05-29)**: `pnpm test:android-emulator` aggregates the direct-adb emulator substitutes and retries only locked debug-launch and session-subscription timing outliers once with the same strict limits. The latest hosted-relay aggregate run on `emulator-5554` passed with locked debug launch `TotalTime=6448ms` (below the default 8000ms limit), pair `pair_flow_ms=1420`, session subscription `visible_ms=5493`, flood screenshot 8437/14400 nonblack samples, and successful background replay, restart restore, multisession, reconnect, and notification tap routing.

**Direct Android adb network-reconnect refresh (2026-05-29)**: a raw `adb`
pass under `/tmp/fieldwork-direct-adb-reconnect-20260529.hkFQik/evidence`
used the AWS live-test relay control plane, isolated release `fieldworkd`,
...
`FIELDWORK_DEBUG_PAIRING_CODE`. The paired run used an isolated release
...
**Direct Android adb locked-launch refresh (2026-05-23)**: a direct headless
`adb` pass started `Medium_Phone_API_36.1`, installed the existing normal debug
APK, resolved `app.fieldwork.android/.MainActivity`, and launched it with
`Status: ok`, `LaunchState: COLD`, and `TotalTime=1847ms`. Evidence under
`/tmp/fieldwork-adb-direct-20260523-locked` includes `locked.png`,
...
**Direct Android adb locked-launch follow-up (2026-05-23)**: another direct
`adb` pass under `/tmp/fieldwork-emulator-direct-20260523` installed the current
normal debug APK on `Medium_Phone_API_36.1`, resolved
...
the UI dump contained only the locked `Unlock` app surface. The Android crash
buffer captured an emulator system `com.google.android.bluetooth` crash, but
targeted logcat and crash-buffer scanning found no Fieldwork `FATAL EXCEPTION`,
ANR, session sync, push-token registration, terminal attach, or input before
unlock. This remains debug-emulator substitute evidence only; physical release
device cold-start and biometric evidence remain unchecked.

**Direct Android adb locked-launch refresh (2026-05-24)**: a direct headless
`adb` pass started `Medium_Phone_API_36.1` as `emulator-5554`, installed the
current normal debug APK, cleared app data/logcat/crash buffers, resolved
`app.fieldwork.android/.MainActivity`, and launched it with `Status: ok`,
`LaunchState: COLD`, `TotalTime=1852ms`, `WaitTime=1854ms`, and measured
`wall_launch_ms=1905`. Evidence under
`/tmp/fieldwork-adb-direct-20260524172604` includes `adb-devices.txt`,
`install.txt`, `launch.txt`, `resolve-activity.txt`, `package-info.txt`,
...
with `nonblack=14379/14400`, the UI dump contained only the locked `Unlock`
surface, and targeted logcat/crash-buffer scanning found no Fieldwork
`FATAL EXCEPTION` or ANR entries. This remains debug-emulator substitute
evidence only; physical release-device cold-start and biometric evidence remain
unchecked.

**Direct Android adb locked biometric fallback refresh (2026-05-24)**: a later
direct headless `adb` pass under `/tmp/fieldwork-direct-adb-20260524220022`
started the same AVD, installed `apps/android/app/build/outputs/apk/debug/app-debug.apk`,
cleared logcat and the crash buffer, and launched
`app.fieldwork.android/.MainActivity` with `Status: ok`, `LaunchState: COLD`,
`TotalTime=2571ms`, and `WaitTime=2606ms`. Evidence includes `install.txt`,
`launch.txt`, `locked.png`, `locked-ui.xml`, `logcat.log`, empty `crash.log`,
`after-unlock-tap.png`, `after-unlock-ui.xml`, `after-unlock-logcat.log`, and
empty `after-unlock-crash.log`. The locked and post-Unlock-tap UI dumps both
contained only `text="Unlock"`; the post-tap logcat showed `BiometricService`
for `app.fieldwork.android` with `Status: 7` and `hasEnrollments: false`; and
targeted scans found no Fieldwork `FATAL EXCEPTION`, ANR,
`FieldworkRepository: listSessions`, `registerPushToken`, `Attached`, or
terminal-content exposure before unlock. This remains debug-emulator substitute
evidence only; physical BiometricPrompt and release-device runtime gates remain
unchecked.

**Direct Android adb pair/input refresh (2026-05-24)**: a direct `adb` pass
under `/tmp/fieldwork-adb-pair-20260524205522` started
`Medium_Phone_API_36.1` as `emulator-5554`, created a desktop-owned
`bash · fieldwork` PTY session in an isolated daemon with
`ANDROID_DIRECT_PAIR_READY`, installed a debug-only biometric-bypass/pair-payload
...
`terminal-after-input.png`, `terminal-after-input-ui.xml`, `logcat.log`, empty
...
contained `bash · fieldwork` and `ANDROID_DIRECT_PAIR_READY`; the attached
terminal UI dump contained `Attached`; Android sent `fw_android_direct_pair_ok`
into the PTY, and a separately approved verifier client confirmed
`android-direct: fw_android_direct_pair_ok`. `pair-buildconfig.txt` proves the
temporary debug-only test build had `FIELDWORK_BIOMETRIC_BYPASS = true` and an
injected pairing code; `restored-buildconfig.txt` proves the APK was rebuilt
and reinstalled back to `FIELDWORK_BIOMETRIC_BYPASS = false` and
...
attached devices. This remains debug-emulator substitute evidence only; physical
Android biometric, QR-camera, Play-signed release build, and release-device
runtime gates remain unchecked.

**Direct Android adb pair/input stale-binary guard refresh (2026-05-25)**: a
follow-up direct `adb` pass under `/tmp/fieldwork-adb-pair-20260524234442`
first exposed a stale `target/release/fieldwork` binary that lacked
`fieldwork doctor`; rebuilding `fieldwork`/`fieldworkd` fixed the local CLI
surface before testing continued. The rerun used an isolated release daemon,
proved `target/release/fieldwork doctor --no-start` with socket hardening,
created the desktop-owned `android_direct` bash PTY, paired Android through the
actual Pair UI plus explicit desktop approval in `pair_flow_ms=841`, listed the
desktop session, attached the terminal, sent
...
desktop attach. Screenshots, UI dumps, logcat, empty crash buffers, and restored
default `FIELDWORK_BIOMETRIC_BYPASS = false` plus empty
`FIELDWORK_DEBUG_PAIRING_CODE = ""` proof were captured; this remains
debug-emulator substitute evidence only. The stale-binary failure mode is now
covered by the live-testing readiness command-surface check above.

**Direct Android adb hosted-relay pair/input refresh (2026-05-29)**: a raw
`adb` pass under `/tmp/fieldwork-direct-adb-pair-20260529.CCfUdt/evidence`
used the AWS live-test relay control plane at `http://3.7.208.153:8443`, an
isolated release `fieldworkd`, and the desktop-created `directshell` bash PTY
session. The debug emulator build was temporarily rebuilt with
`FIELDWORK_ANDROID_BIOMETRIC_BYPASS=true` and the typed pairing code `KPDMT`,
paired through the actual Android Pair surface plus explicit desktop approval,
accepted the Android notification permission prompt, listed `directshell` with
`ANDROID_DIRECT_READY`, attached the terminal, sent `android_direct_ok` through
`adb shell input text`, and a separate desktop attach replay captured
`android-direct: android_direct_ok`. Evidence includes pair/session/terminal
screenshots, UI dumps, app logcat, empty crash buffers, `terminal-replay.txt`,
and `SUMMARY.txt`. The first attempt correctly consumed and denied a code when
the desktop approval prompt was accidentally run without an interactive TTY,
then the test was rerun with a TTY-backed approval session. The default debug
APK was rebuilt and reinstalled afterward, `pnpm check:android-debug-apk`
proved no stale debug pairing code remained, `pnpm test:android-unit` passed,
and the emulator app data was cleared. This remains debug-emulator substitute
evidence only; physical Android biometric, QR-camera, Play-signed release
build, 30-minute renderer dogfood, and release-device runtime gates remain
unchecked.

**Direct Android adb notification-tap routing refresh (2026-05-29)**: a raw
`adb` pass under `/tmp/fieldwork-direct-adb-notify-20260529.OliIrI/evidence`
used the AWS live-test relay control plane, an isolated release `fieldworkd`,
and two desktop-created PTY sessions: `fw_notify_target` and `fw_notify_other`.
The debug emulator build was temporarily rebuilt with
`FIELDWORK_ANDROID_BIOMETRIC_BYPASS=true` and pairing code `MVWNH`, paired
through the Android Pair screen plus explicit desktop approval, and showed both
sessions on the dashboard. A direct `adb shell am start -a
FIELDWORK_OPEN_SESSION ... -e session_id_hash <UPPERCASE_HASH>` did **not**
route to a terminal. A second direct intent with the valid lowercase
`target_session_hash=1e9d64b74d55d80c3163cedd7668c29e1214395274f6d30e1499c6a767bd0604`
opened `fw_notify_target`; Android-originated `notify_tap_ok` reached only the
target PTY input log and did not appear in `fw_notify_other`. Evidence includes
pair/dashboard/invalid-intent/valid-intent/input screenshots, UI dumps, intent
transcripts, app logcat, empty crash buffer, and `SUMMARY.txt`. The default
debug APK was rebuilt and reinstalled afterward, the emulator app data was
cleared, and the isolated daemon was stopped. This remains debug-emulator
substitute evidence; real provider notification delivery, physical lock-screen
tap-through, APNs/FCM payload inspection, and physical release-device routing
remain unchecked.

**Direct Android adb locked/bypass refresh (2026-05-30)**: a raw `adb` pass
under `/tmp/fieldwork-adb-direct-20260530060906` launched the currently
installed normal debug APK on `Medium_Phone_API_36.1` as `emulator-5554` with
`Status: ok`, `LaunchState: COLD`, `TotalTime=5378ms`, and a locked
`Unlock` surface. Tapping `Unlock` stayed locked because the emulator has no
enrolled strong biometric; logcat showed `BiometricService` status `7` for
`app.fieldwork.android` and no Fieldwork fatal/ANR entries. The app was then
rebuilt with the debug-only `FIELDWORK_ANDROID_BIOMETRIC_BYPASS=true`,
launched in `TotalTime=5496ms`, granted camera permission through `adb`, and
relaunched in `TotalTime=3558ms`; UIAutomator then showed the real post-unlock
pairing surface with `Enter code`, `Scan QR`, `Pair`, `Sessions`, and
`Settings`, while CameraX opened normally. Evidence includes launch transcripts,
screenshots, UI XML, focused-window dumps, targeted logcat, empty Fieldwork
crash-buffer captures, and build-config proofs. The default debug APK was
rebuilt and reinstalled afterward with `FIELDWORK_BIOMETRIC_BYPASS = false`
and `FIELDWORK_DEBUG_PAIRING_CODE = ""`, then relaunched in `TotalTime=4884ms`
with the locked `Unlock` surface restored. This remains debug-emulator
substitute evidence only; physical Android biometric, QR-camera, Play-signed
release build, and release-device runtime gates remain unchecked.

**Direct Android adb hosted-relay pair/attach/input refresh (2026-05-30)**: a
raw `adb` pass under `/tmp/fieldwork-adb-hosted-tmux-LmQhEo` used the AWS
live-test relay control plane at `http://3.7.208.153:8443`, a tmux-pinned
isolated release daemon, and a desktop-created `hosted_direct` bash PTY. The
debug emulator APK was temporarily rebuilt with
`FIELDWORK_ANDROID_BIOMETRIC_BYPASS=true`,
`FIELDWORK_DEBUG_PAIRING_CODE = "60TTF"`, and the relay control URL baked in.
Android paired through the real Enter-code Pair surface plus explicit desktop
approval in `pair_flow_ms=1343`, showed `hosted_direct` and
`ANDROID_HOSTED_DIRECT_READY` on the dashboard, attached to the terminal, and
sent `android_hosted_direct_ok` through direct `adb shell input`. A separately
approved verifier client attached over the hosted relay and confirmed replay
contained `android-hosted: android_hosted_direct_ok`. Evidence includes relay
version `contract_version=2`, launch transcripts, screenshots, UI XML,
targeted app logcat, empty Fieldwork crash buffers, and desktop replay proof.
The normal debug APK was rebuilt and reinstalled afterward with
`FIELDWORK_BIOMETRIC_BYPASS = false` and `FIELDWORK_DEBUG_PAIRING_CODE = ""`,
then relaunched with the locked `Unlock` surface restored. This remains
debug-emulator substitute evidence only; physical Android biometric, QR-camera,
Play-signed release build, provider push delivery, and release-device runtime
...
`FIELDWORK_BIOMETRIC_BYPASS = false` and
`FIELDWORK_DEBUG_PAIRING_CODE = ""`; the locked and post-Unlock-tap UI dumps
...
`scripts/create-live-testing-fw-shim.mjs` now also creates the temporary
source-checkout live-test command shim for `fw`, `fieldwork`, and `fieldworkd`
from the repo-local release binaries, with `activate.sh`, `--print-dir`, and
`--print-export` modes so the first live-test round exercises the same short
`fw` command users get from npm. `scripts/test-live-testing-fw-shim.mjs` keeps
that helper covered with fake release binaries, verifies alias preservation, and
confirms the shim does not replace npm package/provenance/release-signing gates.
`pnpm check:live-testing-readiness:local` now also runs
`target/release/fieldwork doctor --help`, `target/release/fieldworkd --help`,
and an internal temporary `fw` shim check for `Usage: fw` plus
	`Usage: fw doctor`, so a stale repo-local release binary or broken alias-aware
	help surface is caught before the direct `adb` live-test pack or
	physical-device preflight. The same preflight now rejects a debug relay-control
	override in `FIELDWORK_RELAY_CONTROL_URL`, so the first physical live-test
	cannot accidentally capture hosted-relay typed-code evidence while claiming the
	local QR handoff path.
...
**Android background/foreground replay note (2026-05-19, refreshed 2026-05-29)**:
`pnpm test:android-emulator-background-replay` pairs the actual Android debug
app with an isolated release daemon through the debug-only QR payload path,
opens a desktop-created terminal, backgrounds the attached app while the PTY
emits `ANDROID_BACKGROUND_REPLAY_OUTPUT`, foregrounds back to `Attached`, sends
`after_background_ok`, and verifies the background-emitted output plus
post-foreground input through a separately approved verifier. A raw adb refresh
under `/tmp/fieldwork-direct-adb-bg-fix-20260529.Om7Krm/evidence` used the AWS
live-test relay control plane, typed pairing code `R8X09`, explicit desktop
approval, and session `bg_replay_fix`. The run sent `before_background_ok` from
Android, backgrounded Fieldwork with `adb shell input keyevent KEYCODE_HOME`,
emitted `ANDROID_BACKGROUND_REPLAY_OUTPUT` from a desktop attach while Android
was backgrounded, foregrounded back to the same attached terminal with HOT
launch `TotalTime=485ms`, and sent `after_background_ok` from Android. The
first pre-fix run exposed a foreground `transport error: connection lost`
dialog from the dashboard session-list stream; Android now retries that
background subscription quietly instead of surfacing it as a global alert, with
focused `FieldworkViewModel` unit coverage. The patched direct adb run showed
no user-visible transport dialog, no Fieldwork fatal/ANR entries, and an empty
crash buffer; the default debug APK was rebuilt/reinstalled afterward and
emulator app data was cleared. This is still emulator substitute evidence; the
release gate remains unchecked until the same behavior is observed on physical
release devices.
...
|---|---|---|---|---|
| 1 | Oracle ARM A1 capacity-grab fails for weeks | High | Medium | Oracle-only mitigation: keep the Mumbai network state, poll OCI capacity reports across all fault domains, apply immediately when A1 capacity appears, and add another Oracle region only if the tenancy's one-region subscription limit is raised. No non-Oracle bridge host is a v1 fallback. |
...
| 10 | Honeycomb free tier exhausted in first month | Low | Low | Lower relay sample rate, trim nonessential aggregate spans, or temporarily export only error-rate spans. |
...
| 12 | npm package squat on platform child names | Low | High | Mitigated 2026-05-26: the operator-owned unscoped `fieldwork` meta package exists, and placeholder `0.0.0` publishes now reserve `fieldwork-darwin-arm64`, `fieldwork-darwin-x64`, `fieldwork-linux-arm64`, and `fieldwork-linux-x64`. `NPM_TOKEN` is set as a GitHub Actions secret on `fieldwork-app/fieldwork` using a 90-day npm granular token expiring 2026-08-24; rotate before release if expired. Final v1 publish must replace the placeholders with Changesets-managed `1.0.0` packages and provenance verification. |
...
| 19 | macOS quarantine/ad-hoc signing blocks npm-installed binaries on some machines | Medium | Medium | Treat this as an npm-native packaging problem, not an Apple-account blocker: verify package integrity, `chmod 0755`, ad-hoc sign with `codesign --force --sign -`, strip quarantine only from verified Fieldwork executables, then prove `fw doctor` and `fieldwork daemon install` preflight behavior on a clean Mac. Document that notarized Mac app/pkg polish is deferred until Apple credentials exist. |
...
| 15 | macOS signing without buying Apple Developer Program | **Desktop npm path ships without Developer ID notarization.** Use npm integrity, release checksums/provenance, ad-hoc `codesign --sign -`, targeted quarantine cleanup, and explicit `fw doctor` reporting. Apple Developer ID remains optional/deferred for notarized Mac app/pkg artifacts and mandatory only when iOS/TestFlight resumes. | Section 9 |
...
- [x] **Operator: confirm npm publish rights for the platform child package family** — completed 2026-05-26. The unscoped `fieldwork` meta package is operator-owned, and placeholder `0.0.0` publishes now reserve `fieldwork-darwin-arm64`, `fieldwork-darwin-x64`, `fieldwork-linux-arm64`, and `fieldwork-linux-x64`. `NPM_TOKEN` is set as a GitHub Actions secret on `fieldwork-app/fieldwork` using a 90-day npm granular token expiring 2026-08-24; rotate before release if expired. Before the actual v1 release, run `node scripts/verify-npm-registry-state.mjs --expect-meta-published --expect-platform-published --expect-latest-version=1.0.0 --expect-provenance` after publishing the real `1.0.0` packages.
...
- [x] Operator: create GitHub org `fieldwork-app`; create empty `fieldwork` repo inside it — completed 2026-05-26 as public `https://github.com/fieldwork-app/fieldwork`, and the local `origin` remote now points at that repo.
- [ ] Operator: reserve `@fieldworkdev` on Twitter/X.
- [ ] Open an Oracle Cloud account (slow approval, do early); add a credit card (won't be charged on Always Free). Account access is now unblocked in tenancy `hridaimadhukar` with home region `India West (Mumbai)`. A dedicated `fieldwork` compartment, local OCI `FIELDWORK` API-key profile, ignored Mumbai tfvars, and relay deploy SSH key are prepared outside git; `RELAY_SSH_KEY` is set on `fieldwork-app/fieldwork`. Terraform created the VCN, public subnet, internet gateway, route table, and security list on 2026-05-28, but `VM.Standard.A1.Flex` instance creation failed with OCI `500-InternalError, Out of host capacity` in the only available Mumbai AD. The tenancy is currently limited to one subscribed region, so `ap-hyderabad-1` subscription returned `TenantCapacityExceeded`. The Terraform scaffold now supports explicit `fault_domain`; `infra/oracle/watch-a1-capacity.sh` is the current Oracle-only path, polling OCI A1 capacity reports for all three fault domains every 10 seconds and applying immediately when capacity appears.
- [x] Defer Apple Developer Program purchase. It is **not required** for desktop npm live testing/release; revisit only when iOS/TestFlight work resumes or when the project intentionally adds notarized Mac app/pkg artifacts.
- [x] Create Firebase project, Android app config, and relay FCM sender — Firebase project display name `Fieldwork`, project id `fieldwork-oss`, project number `368996637975`, Android app id `1:368996637975:android:851c0de94f997e6d4bab52`, package `app.fieldwork.android`. The exact project id `fieldwork` was not available globally, so the display name carries the product name and `fieldwork-oss` carries the project id. `ANDROID_GOOGLE_SERVICES_JSON` is set as a GitHub Actions secret on `fieldwork-app/fieldwork`, and the relay-only `fieldwork-relay-fcm@fieldwork-oss.iam.gserviceaccount.com` service account has `roles/firebasecloudmessaging.admin` for FCM HTTP v1 sends. Its user-managed key is installed only on the AWS live-test relay under `/etc/fieldwork/secrets/fcm-service-account.json`; no provider credential is committed to the repo.
